...
- **Hosting.** The four cash crops host **Floodplain** (capacity 205 ≥ the 195 field-rung floor, so
  they contest grain on real sowable ground) + RollingHills/MixedWoodland, and are kept **off**
  AlluvialPlain/RiverDelta so those baskets' staples (wild_emmer / reed_and_root) stay dominant on
  their best country (the S1 commit-worthiness bar, `flora_roster.rs` + `flora_commitment.rs`). Trade
  rates are **playtest dials**.
...
- **Wire (append-only):** `FloraShareInfo.sowTradePayoff` — trade/turn a sown Field of this plant
  ...
  follow-up:** the native reader must decode `sowTradePayoff` and the crop picker render a cash-crop
  trade row + a cash badge (not done here).
