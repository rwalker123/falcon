...
| `work-party.md` | The work party — hunt and forage are one model: a far row posts a party instead of lapsing, distance is paid in WALKING (a caravan: one hunter walks each full pack home and back), the walk is measured from the apron and a road shortens it, the take flows HOME always, one forecast function serves the turn, the seed and the query, and a posting the band cannot supply walks home | `work_party.rs`, `systems/labor.rs`, `supply.rs`, `labor_config.rs`, `forecast_query.rs`, `tests/labor_allocation.rs`, `tests/work_party_caravan.rs` |
