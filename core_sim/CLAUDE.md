...
| `ai-driver.md` | The AI player process — the claim · greet · hold · reconnect · resync wire sequence and the constants it restates, `SeatView` and the chain-break resync, the turn loop's always-`ready` rule and decide budget, the scripted brain's line format and substitutions, the `seats.roster` launcher contract, the two instruments (`scoreboard.jsonl`, `decisions.jsonl`) and the `sim_ai bench` harness — its measures, `--compare`/`--check`, and the all-Pass baseline ratchet — and the real-sockets scenario and bench tests | `sim_ai/**`, `tests/ai_seat_scenario.rs`, `tests/ai_bench.rs`, `tests/common/ai_process.rs` |
