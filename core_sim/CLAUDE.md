...
| `band-names.md` | A band's name is identity, not a row number — the per-faction permutation, the four founding sites (mint at two, inherit at two), the wire field | `band_names.rs`, `components.rs`, `data/band_names.json`, `tests/band_names.rs` |
