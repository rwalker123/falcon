...
- **S1 changes no rung payoff dial** — `tended_regrowth_gain` and `field_provisions_per_biomass` are
  untouched, deliberately, so any balance movement is attributable to the roster alone. Retuning rung
  2 with competitor-removal now explicit is **S2**.
