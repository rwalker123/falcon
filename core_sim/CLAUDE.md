...
| `routes.md` | Roads as the ladder's third branch — a road is in the ground and does not follow the camp, traffic is the crew, the `length x terrain` span, and what a rung buys | `routes.rs`, `intensification_ladder.json`, `supply.rs`, `tests/route_traffic.rs` |
