...
share }]`, resolved through `tile_flora_composition`. **Derived from the tile's biome, not per-patch
state**: every tile of a biome publishes the same basket, and no `ForagePatch` field feeds it. A patch
*committed* to one species by `Cultivate`/`Sow` is a later slice and gets its own field.
