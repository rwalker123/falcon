...
| `ai-driver.md` | The AI player process — the claim · greet · hold · reconnect · resync wire sequence and the constants it restates, `SeatView` and the chain-break resync, the turn loop's always-`ready` rule and decide budget, the scripted brain's line format and substitutions, the `seats.roster` launcher contract, and the real-sockets scenario | `sim_ai/**`, `tests/ai_seat_scenario.rs` |
