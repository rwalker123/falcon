...
| `launcher.md` | The packaged game's supervisor — one process per locally-hosted seat (the host's human included, and no in-process fast path for it), rival seats left vacant, the readiness gate, and every child reaped by the `Drop` guard plus the Windows job object | `launcher/src/main.rs`, `launcher/Cargo.toml` |
