...
| `starting-loadout.md` | The outfitting window — one per BAND, the grant/take supplies, freshest-first take_units, the onward-take refusal | `starting_loadout.rs`, `systems/fission.rs`, `tests/{starting_loadout,split_loadout}.rs` |
...
| `factions.md` | Who plays the world — the roster as **one human plus the N AI factions the new game asked for**, the map-derived ceiling and default (and the query the client learns them through), positional faction ids, human/AI control, the four command-authorization gates (a QUESTION is seat-gated too), **seats** (a connection claims the faction it drives, and the turn waits for the occupied ones), per-faction placement and its separation lever, the per-faction `StartLocation`, one opening-loadout window per faction, the defection contact gate, **which frame sections are viewer-scoped and which are world-visible** (the three-tier band filter, why a derived aggregate is faction-keyed data, the three exemptions, why fog is not a disclosure switch), the save-wins-over-config rule, and the single-faction assumptions still live | `orders.rs`, `seats.rs`, `start_profile.rs`, `data/start_profiles.json`, `data/simulation_config.json`, `metrics.rs`, `victory.rs`, `bin/server.rs`, `systems/mod.rs`, `systems/worldgen.rs`, `systems/population.rs`, `starting_loadout.rs`, `snapshot/`, `great_discovery.rs`, `knowledge_ledger.rs` |
...
| `extraction.md` | Wood and stone get a producer — a deposit is a stock with a capacity and a regrowth rate and rock's rate is zero, the forestry/extraction branches, the take that pays a material and no food | `extraction.rs`, `extraction_config.rs`, `data/extraction.json`, `tests/extraction.rs` |
| `work-party.md` | The work party — hunt and forage are one model: a far row posts a party instead of lapsing, distance is paid in WALKING (a caravan: one hunter walks each full pack home and back), the walk is measured from the apron and a road shortens it, the take flows HOME always, the home band feeds its party through ordinary consumption so the whole take walks home, and one forecast function serves the turn, the seed and the query | `work_party.rs`, `systems/labor.rs`, `supply.rs`, `labor_config.rs`, `forecast_query.rs`, `tests/labor_allocation.rs`, `tests/work_party_caravan.rs` |
...
| `save-game.md` | The save blob (magic + header + gzipped payload), the four wire verbs, slot files and header-only listing, what a load owes beyond restoring the world, the config-drift warning, the autosave cadence | `save.rs`, `save_store.rs`, `config_fingerprint.rs`, `server.rs`, `command.proto` |
...
| `launcher.md` | The packaged game's supervisor — one process per locally-hosted seat (the host's human included, and no in-process fast path for it), the seat supervisor that starts one `sim_ai` per rival the server's `seats.roster` event names and reaps the departed, the human's client as the session window, the readiness gate, and every child reaped by the `Drop` guard plus the Windows job object | `launcher/src/main.rs`, `launcher/Cargo.toml` |
| `ai-driver.md` | The AI player process — the claim · greet · hold · reconnect · resync wire sequence and the constants it restates, `SeatView` and the chain-break resync, the turn loop's always-`ready` rule and decide budget, the scripted brain's line format and substitutions, the `seats.roster` launcher contract, the three instruments (`scoreboard.jsonl`, `decisions.jsonl`, `observations.jsonl`) and the `sim_ai bench` harness — its measures, `--compare`/`--check`, and the all-Pass baseline ratchet — the run viewer page, the server's run record (`SIM_RECORD_DIR`) and `sim_ai import-record`, and the real-sockets scenario, bench and record tests | `sim_ai/**`, `record.rs`, `tests/ai_seat_scenario.rs`, `tests/ai_bench.rs`, `tests/ai_record_import.rs`, `tests/common/ai_process.rs`, `tests/common/seat_harness.rs` |
...
| `SIM_SAVE_DIR` | Where save slots are read and written. Defaults to `./saves`, relative to the server's working directory. |
| `SIM_RECORD_DIR` | Record the session for the run viewer: every frame published to every seat and every logged command, written under this directory by a dedicated thread that never holds the turn or the publisher. The launcher sets it to `<data_dir>/runs/<run_id>/record`; `scripts/run_stack.sh` sets it by default to `<checkout>/runs/run-<start secs>-<pid>/record` (same `run-` naming, newest 5 runs kept, `/runs/` gitignored) so `sim_ai viewer <run dir>` finds `record/` where it looks, points `runs/current-run` at that run, and prints the `import-record` / `viewer` lines for the human seat (faction 0) when the game ends — the client's exit (a `--client-only` terminal reads the pointer) or a `--server-only` Ctrl-C; `--no-record` opts out and removes the pointer; unset, nothing is recorded. The format and the reader (`sim_ai import-record`) are `.claude/rules/core_sim/ai-driver.md`. |
| `SIM_AI_BRAIN` | Read by the **launcher**, not the server: the `--brain` it names on every `sim_ai` rival it spawns (`pass`, `scripted`, `utility`). Overrides the launcher's `utility` default for one run, so rivals change brain without a rebuild; empty or unset keeps the default. `sim_ai`'s own default is `pass`, the bench's control arm, which is why the launcher always names one — see `.claude/rules/core_sim/launcher.md`. |
