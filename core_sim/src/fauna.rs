...
pub struct Herd {
    ...
    ///
    /// **It is a PROJECTION when nothing is being built** — the same rule `corralYield` beside it
    /// follows, and for the same reason: the pre-commit Corral row is by definition looking at a herd
    /// nobody is penning yet. With a verb in
    ...
    /// so the sim answers — the *sim answers, the client does no arithmetic* discipline. Transient
    /// per-turn scratch on
    ...
    /// = the pasture feeds the pen for free; `0.0` = a barren footprint must be fed entirely on hay,
    /// and starves for whatever the hay does not cover. Exported as `penPastureFraction`, and it is
    /// the LAND half of the feed split — hay is the rest ([`Self::fodder_draw`]), in the same fodder
    /// units, against the same `fodder_per_biomass × biomass` demand. `0.0` for an unpenned herd.
    ...
    /// Foddering, or a pen whose footprint already fed it. Exported as `fodderDraw` — with
    /// [`Self::pen_pasture_fraction`] it is the whole of the feed split ("fed by pasture NN% · hay
    ...
    // **RETIRED: `pen_larder_bill` and `pen_hay_food`** — the two FOOD-unit terms of the old
    // three-way feed split (`gross × pen_pasture_fraction + pen_hay_food + pen_larder_bill == gross`).
    // Both existed only to price a pen's feed in the units the PEOPLE eat in: the larder bill was the
    // human food a keeper handed its livestock, and `pen_hay_food` was hay converted into the food it
    // *displaced* from that bill. A pen is fed grass and hay and nothing else now, so the feed split
    // is `footprint_intake + fodder_draw` against `fodder_per_biomass × biomass` — one quantity in one
    // unit, and [`Self::pen_pasture_fraction`] is the whole of the land/hay share the client needs.
    ...
    /// Transient per-turn flag: the fraction of the pen's **feed** demand its grass and hay covered
    /// last turn — `(footprint_intake + fodder_draw) / (fodder_per_biomass × biomass)`, clamped
    /// `[0, 1]`; `1.0` = fully fed, and the value when nothing was demanded. **One quantity in one
    /// unit**: it used to add a fodder-unit land share to a food-unit larder payment, which is how
    /// human food came to be animal feed at all.
...
// **RETIRED: `pen_upkeep`** — `upkeep_per_biomass × biomass`, the food a pen drew from its keeper's
// larder, answered for every herd (a projection when unpenned) and exported as `penUpkeep`. **Human
// food is not animal feed.** A pen eats what a herd always ate — `fodder_per_biomass × biomass` of
// grass, off its fenced footprint and out of the hay store — so there is no food-unit running cost
// left to quote, and nothing to subtract from `corral_yield` in a pre-commit preview. What a pen can
// still cost its keeper is *work* (`herders_needed`, the `animal:pen` rung's standing upkeep) and the
// hay a fodder Field has to grow; what it can never cost is bread.
...
/// husbandry density gain reaches it: hay does not make the field bigger and neither does
/// domestication, so both stay on the feed path and this term is purely *how much ground there is*.
///
/// # ⛔ AN AREA BUDGET, SPENT AT `body_mass^(2/3)` PER ANIMAL
///
/// A hex holds the same **area** of animal, not the same **mass** of it: footprint scales with the
/// 2/3 power of mass (area goes as length², mass as length³), so a fowl at 1/900th an aurochs'
/// weight takes about 1/95th of its floor space. The two steps below are deliberately left
/// unsimplified — `budget / body^(2/3)` animals, then `× body` kilograms each — because the
/// algebraically equal `budget × body^(1/3)` reads as a bare cube root that states nothing about
/// why.
///
/// **`tiles` is the herd's own footprint** — the fenced disk for a pen, the roam disk for a mobile
/// herd — counted by the same `hex_range_tiles` the feed sum walks, so a footprint clipped by a map
/// edge really is smaller.
///
/// A non-positive `body_mass` (unreachable — `FaunaConfig::validate` pins it positive) answers
/// [`NO_SPACE_CAP_AT_ALL`] rather than dividing by zero: *"the roster does not describe this animal"*
/// is not *"no animal fits"*.
pub fn herd_space_capacity(footprint_tiles: usize, body_mass: f32, fauna: &FaunaConfig) -> f32 {
    let Some(budget) = fauna.husbandry.hex_space_budget else {
        return NO_SPACE_CAP_AT_ALL;
    };
    if !body_mass.is_finite() || body_mass <= 0.0 {
        return NO_SPACE_CAP_AT_ALL;
    }
    // **How many of THIS animal one hex has room for** — the area each one occupies is its mass to
    // the 2/3, and the budget is how much of that area a hex has to spend.
    let animals_per_tile = budget / body_mass.powf(ANIMAL_FOOTPRINT_EXPONENT);
    // …and the biomass that many animals come to, over the whole footprint.
    animals_per_tile * body_mass * footprint_tiles as f32
}

/// **AREA FROM VOLUME** — an animal's floor space grows as the 2/3 power of its mass, because area
/// goes as length² and mass as length³. Named rather than written `2.0 / 3.0` inline so the relation
...
fn ecological_carrying_capacity(
    ...
    // **THE SPACE HALF, AND IT APPLIES TO EVERY DIET AND EVERY RUNG** — how much biomass this herd's
    // physical footprint holds ([`herd_space_capacity`]). Feed is soft and fodder may offset it;
    // space is hard and nothing offsets it, so the two are `min`'d rather than blended. Resolved off
    // the same `herd_footprint_at` the graze sum below walks, before the carnivore branch returns, so
    // a wolf's ground is measured by the same expression a cow's is.
    let (anchor, radius) = herd_footprint_at(herd, def, at.penned);
    let range = hex_range_tiles(anchor, radius, width, height, wrap);
    let space = herd_space_capacity(range.len(), herd.body_mass, fauna);
    ...
    // **`min(feed_K, space_K)`** — and the density gain is applied to the feed side ONLY, above the
    // `min`, because domestication makes the land hold more animals *per unit of feed* and does not
    // enlarge the ground.
    Some((flow / herd.fodder_per_biomass * herd_density_gain(&at.standing, herd, fauna)).min(space))
}
...
/// animal-edible mirror of `forage::forage_take`. A corralled herd eats its **fenced footprint**
/// instead (`advance_pen_grazing`), on a different draw, so it is skipped here.
///
...
pub struct SourceYieldForecast {
    ...
    /// A **penned** herd reads the same pen MSY: with the pen's harvest drawn down like every other
    /// rung's, that long-run rate is what the pen settles at, so the payoff line and the take agree
    /// at the operating point rather than being two shapes. Lets the client show the payoff
    /// ("preparing X → then Y") *before* the player commits to the dip.
    ...
    ///
    /// **[`f32::INFINITY`] on a continuous source** — a plant is not stalked, so there is no
    /// engagement stage at all. Same shape as `body_mass_yield`, which is likewise inert on the
    /// plant web.
    ///
    /// **A PEN CARRIES A REAL RATE**: the keepers' handling ([`herd_engage_rate`], the species' own
    /// rate times `husbandry.pen_engage_gain`), because the tend branch is bounded by how many
    /// animals a crew can walk out and butcher in a turn. An infinite bound was never *"a fenced
    /// animal is not stalked"*, it was no bound.
    pub engage_rate: f32,
    ...
    ///
    /// **`None` = there is no engagement to resolve at all** — every plant source. The same statement
    /// `engage_rate: f32::INFINITY` makes beside it.
    ///
    /// **A PEN CARRIES `Some`, and [`Self::slaughters`] is what marks it.** This tuple is read by the
    /// *retreat* as well as the fight — `quarry.profile.wariness` is the pen's own `pen_wariness` —
    /// and §4.10 ② gave the pen a real retreat deliberately. Nulling it to express *"no fight"* would
    /// delete the retreat with it, which is the blanket exemption that change retired.
    pub fight: Option<(HuntingParty, QuarryFight)>,
    /// **IS THE KILL ARM A SLAUGHTER RATHER THAN A FIGHT?** — `true` at `animal:pen` and nowhere else
    /// ([`herd_fight_stage`]).
    ///
    /// > *"No fight in a pen, you slaughter it."*
    ///
    /// Everything the keepers reach and that does not break off goes down: no attack-vs-defense gate,
    /// no durability grind, no wound ledger, no casualties, no weapon wear. **The engagement bound and
    /// the escapement clamp are untouched** — the take is still capped by the keepers' handling rate
    /// ([`herd_engage_rate`]) and by the room above the floor, and still quantised to whole animals.
    ///
    /// It is a field of its own rather than a `None` on [`Self::fight`] because the two facts are
    /// about different stages, and one of them is still live at a pen.
    pub slaughters: bool,
    /// **DOES CARRY BIND THIS SOURCE AT ALL?** — `true` only at `animal:pen`, and only while
    /// `husbandry.pen_is_a_larder` is on ([`herd_collection`]).
    ///
    /// A penned herd is a larder on the hoof: you slaughter what you need and the rest stays alive
    /// behind the fence, so nothing has to be hauled and nothing is left to rot. The forecast carries
    /// it because the preview must run the take's own bounds or it is quoting a different source
    /// (`.claude/rules/core_sim/yield-forecast.md`).
    ///
    /// It is separate from [`Self::slaughters`] for the reason that one is separate from
...
// **RETIRED: `NO_FIGHT_STAGE`** — the `Option<(HuntingParty, QuarryFight)>` sentinel a pen and a
// plant patch shared. The pen half went with the exemption it named (`docs/plan_standing_upkeep.md`
// §4.9 item 12b): every animal source resolves a fight now, at every rung. The plant half never
// needed a name — `forage.rs` writes a literal `fight: None` beside its `engage_rate:
// f32::INFINITY`, and *"a plant is not stalked"* is already stated there.

/// **The retreat term of a source that has no retreat stage** — the **plant web**, which
/// [`SourceYieldForecast::fight`] answers `None` for (a plant does not bolt).
///
/// `1.0` is the identity, and there it is the **answer** rather than a filler: nothing walks away
/// from a gatherer, so the whole of a patch's offer reaches the take. The plant web cannot reach a
/// stay at all in any case, its `engage_rate` being [`f32::INFINITY`].
///
/// **A PEN IS NO LONGER ONE OF THEM.** A fence *calms* the animals (`husbandry.pen_wariness`) rather
/// than deleting the stage — see [`herd_wariness`] — so a penned herd passes its own scaled retreat
/// like every other rung.
///
/// **It is not a stand-in for an unresolved party** — a hunting source must pass its own
/// [`HuntingParty::stay_fraction`], or the crew stops matching the take.
pub(crate) const NO_RETREAT_STAGE_STAY: f32 = 1.0;

/// **THE ANIMALS A QUANTISING SOURCE WITH NO FIGHT BRINGS DOWN: none.** Named rather than a bare
/// `0.0` because it states a *structural* fact rather than a measurement — since
/// `docs/plan_standing_upkeep.md` §4.9 item 12b every quantising source carries a
/// [`SourceYieldForecast::fight`] (the pen's exemption is retired), and the one producer that still
/// writes `fight: None` is the plant web, which does not quantise. So this arm is reachable only
/// from a hand-built fixture, and `0` is what such a source honestly takes.
pub(crate) const NO_QUANTISED_SOURCE_WITHOUT_A_FIGHT: f32 = 0.0;
...
/// projects the collection its tend branch runs — the keepers' handling against the room, with no
/// retreat and no fight, since a penned beast is walked out rather than stalked. Reuses the shared
/// model helpers ([`regrow_biomass`], [`hunt_take_room`], [`herd_engage_rate`], [`HuntYield::apply`],
...
/// A source whose room clears a body every turn — fast game, and most pens — simply has a positive
/// value in every slot, which the client draws as a solid run.
///
...
/// the take path uses ([`regrow_biomass`], [`hunt_take_room`], [`quantise_animal_take`],
/// [`resolve_hunt_fight`], [`HuntYield::apply`], [`herd_ecology`]/[`herd_capacity`]) —
/// **no second copy of the take math**, so the schedule is what the sim will really pay.
...
pub fn project_arrivals_hunt(
    ...
    // this herd is its own allocation with its own hands and scales nothing here.
    //
    // **The keepers' handling on a pen** — [`herd_engage_rate`], which folds the pen gain in and
    // hands the wild rate back on an un-penned herd, so the twin in [`project_realized_hunt`] and
    // this one read one term at every rung. [`herd_wariness`] beside it is the retreat's own rung
    // seam, the same stepped shape.
    let reach = animals_engaged(workers, herd_engage_rate(&quarry, fauna));
    let wariness = herd_wariness(&quarry, fauna);
...
fn forecast_production_and_take_at(
        ...
        .flatten()
    {
        Some(axis) => {
            ...
            // **What this crew can bring into contact in a turn** — a party's stalking reach on a
            ...
            let brought_down = match &forecast.fight {
                ...
                Some((party, quarry)) => {
                    ...
                    // floor bounds what the party *goes after*, exactly as `systems::hunt_take`
                    // bounds it (§1). Clamping here rather than leaving it to the quantiser is what
                    // keeps the retreat running on the same population the take retreats; see
                    // [`animals_affordable`]. Both terms are read on `axis`, and an animal count is a
                    // ratio, so the currency cancels.
                    let engaged = reach.min(animals_affordable(
                ...
                // pen's exemption is retired — and the only `fight: None` producer left is the plant
                // web, which does not quantise and therefore takes the continuous arm below.
                //
...
pub struct HuntFight {
    ...
    /// A source with **no fight stage at all** — the plant web alone since §4.9 item 12b — never
    /// reaches [`resolve_hunt_fight`]: it is continuous, so it hands the quantiser nothing and takes
    /// the un-quantised arm instead.
...
///
/// A non-finite or non-positive `stayed`, or a party of nobody, is returned unchanged with no fight
/// and no injuries at all.
///
/// **A PEN NEVER REACHES THIS FUNCTION.** It is routed to [`slaughter`] by [`resolve_hunt_kill`], on
/// [`herd_fight_stage`]'s answer — which is what the sentence that used to stand here (*"a penned
/// animal is not stalked, not fought, not wary and not dangerous"*) described back when the pen
/// expressed its exemption as an infinite engagement bound. Two thirds of that sentence are now
/// false: a penned animal IS stalked (the keepers' handling rate bounds the take) and IS wary
/// (`pen_wariness`). Only *not fought* and *not dangerous* survive, and they survive one seam over.
pub fn resolve_hunt_fight(
    ...
    if !stayed.is_finite() || stayed <= 0.0 || hunters <= 0.0 {
        return HuntFight {
            brought_down: stayed,
            // No fight ran, so the rate is the bound itself — `<= 0` for an empty engagement, and
            // a non-finite one for a caller that passed no bound. Quantising nothing and rating
...
/// ([`resolve_hunt_engagement`], [`animals_affordable`]) — at **every** rung since §4.9 item 12b,
/// the pen included — so the arm was provably dead on every path, and the one caller that had **no**
/// bound of its own (the pen, back when it collected through a helper of its own) stayed invisible
/// for exactly as long as this one silently covered for it. Restraint is free precisely because of
/// that ordering (`docs/plan_hunt_through_combat.md` §1).
///
...
// **RETIRED: `animals_handled(handling, policy_ceiling, body_mass)`** — the pen's own collection
// stage, `min(floor(handling), animals_affordable(ceiling))`, and the last piece of the separate
// take path a fenced herd used to run.
//
// It went with that path (`docs/plan_standing_upkeep.md` §4.9 item 12b). A pen is taken through
// `systems::hunt_take` now, so its two surviving terms are already there and stated once for every
// rung: the room clamp is `reach.min(animals_affordable(ceiling))` inside
// [`resolve_hunt_engagement`], and the whole-animal floor is [`resolve_hunt_fight`]'s. What this
// function could not express is the two stages the pen was exempt from — the retreat and the fight —
// which is exactly why it had to go rather than gain arguments.
//
// A gravestone rather than a shim: every former call site holds a `&Herd`, so keeping a
// fightless collection helper around is precisely how one rung would quietly stop fighting again.
...
/// [`ANIMAL_COUNT_EPSILON`], factored out so a rate expressed in animals and one expressed in
/// biomass round on one rule rather than two copies of it.
...
pub fn quantise_animal_take(
    ...
    // animal you never reached. It arrives already **whole** — [`resolve_hunt_fight`] floors it at
    // every rung — so `killed` below stays integral and `killed_biomass` cannot disagree with the
    // reported count.
    //
    ...
    AnimalTake {
        // `killed` is the animals a fight put on the ground — a finite whole count bounded by the
        // source's own room at every rung, so the cast is safe.
...
pub fn hunt_take_bound(
    ...
    if affordable <= carryable.min(brought_down) {
        ...
        HuntTakeBound::Carry
    // **BOTH SIDES IN WHOLE ANIMALS.** `brought_down` is always whole (the kill arm floors it at
    // every rung); `stayed` is not — a party whose reach is a *fraction* of an animal stays a
    // fraction. Comparing the two raw made the strict inequality fire on the rounding remainder, so
    // a party that never brought a whole animal into contact was reported as having LOST A FIGHT it
    // never had: a lone hunter reaching 0.05 of a mammoth read `Fight` with attack 20 against
    // defense 12, and a pen — which has no fight stage at all ([`slaughter`]) — read `Fight` on
    // every turn its keepers handled a fractional animal. Flooring makes the arm say what it means:
    // brought down everything whole that stayed ⇒ **reach** was the limit.
    } else if brought_down < stayed.max(0.0).floor() {
        HuntTakeBound::Fight
    } else {
...
///
/// `0` for the **plant web**, whose `engage_rate` is [`f32::INFINITY`] — a plant is not stalked — so
/// the `max()` collapses to the haul term and that web does not regress. Same for a degenerate
/// `body`/rate.
///
/// **A PEN DOES REPORT ONE**, and it is the keepers rather than the hunters: its `engage_rate` is the
/// handling rate ([`herd_engage_rate`]) and its `stay` is [`NO_RETREAT_STAGE_STAY`], so this term
/// answers *"how many keepers to walk the room out"* — a real bound on the tend branch's take.
///
...
pub fn herd_engage_rate(herd: &Herd, fauna: &FaunaConfig) -> f32 {
    ...
    if herd.is_corralled() {
        // **THROUGH THE PER-SPECIES SEAM** ([`FaunaConfig::pen_engage_gain_for`]) — this animal's own
        // handling gain where the roster states one, the global where it does not. One keeper handles
        // far more chickens in a day than cattle, and a flat gain across the roster says otherwise.
        wild * fauna.pen_engage_gain_for(&herd.species)
    } else if herd.is_domesticated() {
        // **A HALTER BUYS REACH TOO** (`husbandry.pastoral_engage_gain`). It did not, and that was
        // the hole: rung 2 raised the breeding rate and the retreat and nothing that *bound*, on a
        // web where no shipped source is limited by its herd running out. See the field's own doc.
        //
        // **THROUGH THE PER-SPECIES SEAM** ([`FaunaConfig::pastoral_engage_gain_for`]) — this animal's
        // own gain where the roster states one, the global where it does not. Reach binds the
        // heavy-bodied half of the roster and is inert on the small-bodied half, so a single global
        // number could not describe both.
        wild * fauna.pastoral_engage_gain_for(&herd.species)
    } else {
        wild
    }
}

/// **HOW READILY THIS HERD BREAKS OFF AT CONTACT, at the rung it actually stands on** — the retreat
/// stage's term, and the wariness twin of [`herd_engage_rate`].
///
/// # ⛔ EVERY RUNG RUNS THE RETREAT; THE RUNG ONLY SAYS HOW MUCH OF IT
///
/// A pen used to run no retreat at all ([`NO_RETREAT_STAGE_STAY`] and a `fight: None` forecast), so
/// the ladder was a **mode switch** rather than a tuning: taming and penning bought nothing at the
/// kill, and a fenced animal was collected by a formula the range never used. Management buys a
/// *fraction* of the species' own `wariness` instead — `husbandry.pastoral_wariness` on a halter,
/// `husbandry.pen_wariness` behind a fence, both `(0, 1]` and monotone — so a jumpy gazelle and a
/// placid mammoth stay different animals at rung 3.
///
/// **The FIGHT is on this ladder now, and it is not on THIS dial.** Rung 2 scales the quarry's
/// `defense`/`durability` ([`herd_resistance`]) and rung 3 deletes the fight outright
/// ([`herd_fight_stage`]) — both separate from the retreat, which is what this function answers.
/// Keeping them apart is what lets a haltered animal be *calmer* and *softer* by different amounts.
///
/// **It STEPS at the fence**, exactly as [`herd_engage_rate`] does: `is_corralled()` /
/// `is_domesticated()` are stored facts set at completion, and half a fence is no fence.
///
/// # It is the same shape as [`herd_engage_rate`], and that used to be an asymmetry
///
/// That seam had **no pastoral arm** — *"a halter calms an animal; it does not catch it"* — and the
/// measurement that followed showed the rung it left empty: taming raised the breeding rate on a web
/// where no source is bound by its stock, so rung 2 bought nothing that bound. `pastoral_engage_gain`
/// closes it, and the two seams now step at both rungs in step.
pub fn herd_wariness(herd: &Herd, fauna: &FaunaConfig) -> f32 {
    let wild = fauna.wariness_for(&herd.species);
    if herd.is_corralled() {
        wild * fauna.husbandry.pen_wariness
    } else if herd.is_domesticated() {
        wild * fauna.husbandry.pastoral_wariness
    } else {
        wild
    }
}
...
///
/// **It is the quarry AT ITS RUNG in one other respect: `wariness`.** The species table's row is the
/// wild animal's, and the retreat is the one combat term husbandry tunes ([`herd_wariness`]) — so
/// the `CombatStats` handed on carries the rung-scaled value while `defense`, `durability` and
/// `attack` stay the species' own. Folding it here rather than at each reader is what keeps
/// [`forecast_production_and_take_at`], which holds a `SourceYieldForecast` and no herd, retreating
/// exactly the animals [`resolve_hunt_engagement`] retreats.
pub fn herd_quarry_fight(herd: &Herd, fauna: &FaunaConfig) -> QuarryFight {
    let species = fauna.quarry_fight_for(&herd.species);
    let resistance = herd_resistance(herd, fauna);
    QuarryFight {
        profile: CombatStats {
            wariness: herd_wariness(herd, fauna),
            // **The fight IS on the ladder now, at rung 2** — see [`herd_resistance`]. Both terms
            // move together because they are the two halves of one gate: `defense` decides whether a
            // strike lands at all and `durability` how many landed strikes a body absorbs, and
            // scaling one without the other would make "easier to bring down" mean two things.
            defense: species.profile.defense * resistance,
            durability: species.profile.durability * resistance,
            ..species.profile
        },
        ..species
    }
...
/// **WHAT THIS HERD'S `defense` AND `durability` ARE MULTIPLIED BY, at the rung it stands on** —
/// `husbandry.pastoral_resistance` on a halter, the identity everywhere else.
///
/// # ⛔ PASTORAL ONLY, AND THE PEN'S ABSENCE HERE IS STRUCTURAL
///
/// `animal:pen` has **no fight stage at all** ([`herd_fight_stage`]), so there is nothing at rung 3
/// for a resistance multiplier to scale: a penned animal is slaughtered, and a slaughter has no
/// attack-vs-defense in it. Giving the pen an arm here would be a dial that could never be read,
/// which is worse than an absent one because it would look tunable.
///
/// It is the third thing taming buys, beside `pastoral_gain` (the breeding rate) and
/// `pastoral_wariness` (the retreat) — and unlike those two it moves a term that actually **bounds**
/// the take on the shipped roster.
pub fn herd_resistance(herd: &Herd, fauna: &FaunaConfig) -> f32 {
    if herd.is_domesticated() && !herd.is_corralled() {
        // **This animal's own, or the global** ([`FaunaConfig::pastoral_resistance_for`]) — the arm
        // that reaches the *fight-bound* half of the roster, where reach is inert.
        fauna.pastoral_resistance_for(&herd.species)
    } else {
        NO_RESISTANCE_CHANGE
    }
}

/// **WHAT THIS CREW CAN COLLECT FROM THIS SOURCE IN A TURN, IN BIOMASS** — `workers × per_worker`
/// on every drawn-down source, and [`NO_CARRY_BOUND`] at a pen once
/// `husbandry.pen_is_a_larder` is on.
///
/// # ⛔ THE PEN'S ARM IS AN INFINITY, AND THAT IS THE WHOLE MECHANISM
///
/// Every place carry can bind a take reads this one term — the quantiser's pack seat, the smooth
/// projection's `min`, and [`hunt_take_bound`]'s `carryable` — so making it infinite removes the
/// bound *and* the waste in one statement instead of three edits that could fall out of step.
/// `carried = killed_biomass.min(collection)` becomes an identity, which is exactly *"the meat does
/// not rot, it is still standing in the pen"*.
///
/// **A crew of nobody collects nothing**, larder or not: `0 × ∞` is `NaN`, and a source that nobody
/// is working must answer `0`. That guard is why this is a function rather than a multiplication at
/// each site.
///
/// The **wild** and **pastoral** rungs are untouched at any setting — see
/// [`crate::fauna_config::HusbandryConfig::pen_is_a_larder`].
...
pub const NO_CARRY_BOUND: f32 = f32::INFINITY;

/// A source nobody is working collects nothing — the `workers == 0` answer, which must not be
/// `0 × ∞`.
const NOTHING_COLLECTED: f32 = 0.0;
...
/// *"this rung buys no resistance"* reads as a statement.
const NO_RESISTANCE_CHANGE: f32 = 1.0;

/// **THE KILL ARM'S QUARRY, AT THE RUNG THE HERD STANDS ON — `None` AT A PEN.**
///
/// > *"No fight in a pen, you slaughter it."*
///
/// [`SourceYieldForecast::fight`] has said `None` means *"a pen (a penned animal is slaughtered, not
/// stalked)"* since the field was written, and [`resolve_hunt_fight`]'s own doc says a pen is *"not
/// stalked, not fought, not wary and not dangerous"* — but the take path fought it anyway, because
/// the pen's exemption was expressed as an **infinite engagement bound** and §4.10 ② correctly
/// retired that. Retiring the infinity retired the exemption with it. This is the exemption put back
/// **at the stage it was always about**: the fight, and only the fight.
///
/// # ⛔ WHAT STAYS: THE ENGAGEMENT AND THE ROOM
///
/// A pen keeps its engagement bound ([`herd_engage_rate`], the keepers' handling rate — how many
/// animals a crew can walk out and butcher in a turn) and its escapement clamp ([`herd_take_room`]),
/// and its take is still quantised to whole animals. You still slaughter whole animals, you can
/// still only slaughter as many as the crew can handle, and you can still only slaughter what the
/// herd can spare. **Do not read this as the infinite bound coming back.**
///
/// # It is the OWNER's take, not a raider's
///
/// `systems::expedition_take_biomass` is untouched: a detached party raiding somebody else's pen is
/// not the pen's keeper, and the animals do not stand still for it. That path resolves its own
/// quarry from scalars and never asks this seam.
pub fn herd_fight_stage(herd: &Herd, fauna: &FaunaConfig) -> Option<QuarryFight> {
    (!herd.is_corralled()).then(|| herd_quarry_fight(herd, fauna))
}

/// **THE KILL ARM — a FIGHT where there is one, a SLAUGHTER where there is not.** The one dispatch
/// every take path on the animal web runs, so the pen's exemption cannot be present on one path and
/// missing on another (which is exactly how it went missing the first time).
///
/// `quarry: None` is [`herd_fight_stage`]'s pen answer.
pub fn resolve_hunt_kill(
    stayed: f32,
    hunters: f32,
    party: &HuntingParty,
    quarry: Option<&QuarryFight>,
    // Carried so a slaughter hands the herd's own damage ledger straight back rather than clearing
    // it: an animal wounded on the range and *then* penned keeps its wounds, it is simply no longer
    // fought over them.
    wounds: DamageLedger,
    draw: HuntDraw,
) -> HuntFight {
    match quarry {
        Some(quarry) => resolve_hunt_fight(stayed, hunters, party, quarry, draw),
        None => slaughter(stayed, wounds),
    }
}

/// **EVERYTHING THAT STAYED GOES DOWN** — the pen's kill arm, and the whole of what "no fight" means.
///
/// No attack-vs-defense gate, no durability grind, no wound ledger movement, no casualties, and no
/// strike charges: nobody swings a weapon at a penned animal, so nothing wears out and nobody is
/// hurt. `fought` is `false` for the same reason the one-sided arm reports it.
///
...
pub struct HuntCrewCurveInputs<'a> {
    ...
    /// only thing left that distinguishes them: a pen runs the same engagement, retreat and fight
    /// the range does, and what stops a penned row rising past the kill rate is the keepers running
    /// out of hands to carry the meat home with.
...
pub fn hunt_crew_take_curve(inputs: &HuntCrewCurveInputs<'_>) -> Vec<HuntCrewTake> {
    ...
    //
    // # ⛔ IT ASKS [`herd_collection`], AND A BARE `is_corralled()` HERE WAS A DIVERGENCE
    //
    // `husbandry.pen_is_a_larder` retires the carry bound **at a pen**: what a keeper does not
    // butcher this turn is not wasted meat, it is next turn's stock, still breeding. That flag is one
    // infinity at [`herd_collection`], and its own note names the sites that must read it — the
    // quantiser's pack seat, the smooth projection's `min`, `hunt_take_bound`'s carryable. **This
    // curve is a fourth such site and it was composing its own carry instead**, so a penned row
    ...
    (1..=inputs.max_workers)
        .map(|workers| {
            ...
            // **The keepers' haul, in animals a turn** — `None` on the range, where the haul is a
            // bound the *take* applies afterwards rather than a term of the kill rate. Un-floored
            // like everything else on this curve, save its own [`ONE_WHOLE_ANIMAL`] minimum.
            //
            ...
            // is the one term that makes a penned row differ from a stalking one at all) and because
            // a retune of `durability`, `attack` or a carry tier can walk into it; it is exercised
            // by an authored fixture, `hunt_useful_crew_on_the_wire::
            // a_pens_curve_is_bounded_by_what_its_keepers_can_carry_home`, exactly as
...
// **RETIRED: `pen_crew_take_curve(quarry, inputs)`** — the pen's own crew curve, which priced the
// room, the keepers' handling and their carry with **no retreat and no fight**, and therefore
// published `low == likely == high`.
//
// It went the way every other pen exemption did (`docs/plan_standing_upkeep.md` §4.9 item 12b), and
// it had to: once the tend branch took through `systems::hunt_take`, this was the last reading still
// describing the pre-12b pen. A bare-handed band with a penned aurochs was quoted nothing and paid
// nothing while this curve — behind [`hunt_useful_crew`] and the Work board's `+` — went on
// answering *"another pair of hands buys you more"*. A forecast and a readout disagreeing about one
// row is the whole defect class the slice deletes, restated one level down.
//
// **The two terms it applied are inside [`resolve_hunt_engagement`] already** — the room clamp
// (`herd_take_room` → [`animals_sparable`] at [`EngagementQuantum::Rate`]) and the handling bound
...
/// [`quantise_animal_take`]'s own `carryable.max(1.0)`, named where [`hunt_crew_take_curve`]
/// applies it as a rate on a **penned** row: the carry bound may not fall below one body, though
/// every other bound may.
...
/// same biomass clamp, same biomass→provisions conversion. A **corralled** herd forecasts the take
/// its tend branch resolves, which since §4.9 item 12b is the same three stages the range runs — the
/// keepers' boosted reach, the fence's calmed retreat, and the species' own undiscounted fight. The
/// band Hunt labor has no carry limit
...
pub(crate) fn hunt_forecast(
    ...
    // The pen's yield is **gross**, and now it is also net: a pen's feed is grass and hay, so there
    // is no provisions debit standing against this number at all.
    //
    ...
    SourceYieldForecast {
        ...
        // animals the party can even reach — the keepers' handling rate on a pen, which
        // [`herd_engage_rate`] folds the pen gain into.
        ...
        // turn it finally goes down" on the turn it does (`herd_quarry_fight`, §4.2), and its
        // **rung-scaled `wariness`**, so the retreat this quote runs is the one the take runs.
        //
        // # ⛔ EVERY RUNG FIGHTS, AND THE PEN IS NOT AN EXCEPTION
        //
        // A corralled herd used to forecast [`NO_FIGHT_STAGE`] because its tend branch collected the
        // animals with a formula of its own instead of calling `systems::hunt_take`. Both are gone
        // (`docs/plan_standing_upkeep.md` §4.9 item 12b): the tend branch takes through `hunt_take`
        // like the range does, so the quote runs the same engagement, the same retreat and the same
        // fight the payout resolves. **What the rung buys is the first two stages** — the reach
        // through `husbandry.pen_engage_gain`, the retreat through `husbandry.pen_wariness` — and
        // never the third: a bare-handed band with a penned aurochs is quoted nothing and paid
        // nothing, which is the forecast-vs-actual agreement
        // `.claude/rules/core_sim/yield-forecast.md` asks for, now reached by making the take honest
        // rather than by exempting the quote.
        // **STILL `Some` AT A PEN, AND THE RETREAT IS WHY.** This field carries the quarry the
        // *retreat* reads (`pen_wariness`) as well as the one the fight reads, and §4.10 ② gave the
        // pen a real retreat on purpose. What the pen loses is the fight alone — `slaughters` below.
        fight: Some((party.clone(), herd_quarry_fight(herd, fauna))),
        // **A PEN SLAUGHTERS** ([`herd_fight_stage`]) — the forecast and the take run one kill arm.
        slaughters: herd_fight_stage(herd, fauna).is_none(),
...
///
/// **`pub` because a rung-3 source's published `sustainable` is NOT this line.**
/// [`forecast_source_yield`] overwrites it with the turn's own `actual` on a managed source (a pen, a
/// Field), so the one question *"is the number we are reading a steady rate or a drawdown?"* cannot
/// be answered off the wire at the top of either ladder. `core_sim/tests/food_economy_table.rs`
/// answers it by resolving this — the very expression both previews compute before discarding it —
/// against the take, at every rung.
pub fn sustainable_yield(biomass: f32, cap: f32, ecology: &EcologyConfig) -> f32 {
...
pub fn hunt_take_overdraws(
    ...
    let reach = animals_engaged(workers, herd_engage_rate(herd, fauna))
        * party.stay_fraction(herd_wariness(herd, fauna))
...
pub fn regrow_biomass(herd: &mut Herd, fauna: &FaunaConfig) {
...
mod tests {
    ...
    /// **THE ROOM, SPENT BEFORE THE TAKE** — `resolve_hunt_engagement`'s
    /// `reach.min(animals_affordable(ceiling))` written out, in whole bodies.
    ///
    /// The quantiser fixtures below are about [`quantise_animal_take`] alone, so they need the
    /// room-clamped whole count the take feeds it without standing a herd, a party and a fight up
    /// around it. It was `animals_handled` until the pen's separate collection path retired with the
    /// exemption it served (§4.9 item 12b); it is a **test** helper now precisely so no production
    ...
    fn the_take_bound_names_the_term_that_actually_bound() {
        ...
        for (ceiling, room, collection, stayed, brought_down, expected) in cases {
            ...
            // ([`room_clamped`], `resolve_hunt_engagement`'s clamp), then the quantiser. The
            // quantiser no longer re-applies the ceiling, so a fixture that handed it one would be
            // pricing a pipeline the sim does not run.
            let take_at = |ceiling: f32, collection: f32, brought_down: f32| {
                quantise_animal_take(
                    collection,
                    BODY,
                    room_clamped(brought_down, ceiling, BODY),
                    EngagementStop::WhenPackFull,
                )
            };
    ...
    ///
    /// **The wait now happens where the room is spent** ([`room_clamped`]), not inside the
    ...
    fn a_wait_turn_reports_the_floor_that_caused_it() {
        ...
        let collected = room_clamped(f32::INFINITY, BODY / 2.0, BODY);
        assert_eq!(
            collected, 0.0,
    ...
    fn the_engagement_bound_reaches_the_take() {
        ...
        let bounded = quantise_animal_take(
            collection,
            BODY_MASS,
            room_clamped(engaged, AMPLE_CEILING, BODY_MASS),
            EngagementStop::WhenPackFull,
        );
        ...
        let carry_bound = quantise_animal_take(
            collection,
            BODY_MASS,
            room_clamped(f32::INFINITY, AMPLE_CEILING, BODY_MASS),
            EngagementStop::WhenPackFull,
        );
