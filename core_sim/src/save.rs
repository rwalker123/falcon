...
/// | 10 | `BandBench` gained `last_started` — which recipe the band last started for each thing it makes, the fact the crafting ledger suggests from when an item has several recipes. The whole bench rides `BandRecord::bench`, so a version-9 blob has no such field |
