...
/// | 10 | `LaborAllocation` gained `last_pool_crew` — the four keeping pools' crew accounts, how many of each pool's keepers the turn's bill did not consume, which the wire publishes as `PopulationCohortState.poolCrew`. A band's whole allocation rides `BandRecord::labor`, so a version-9 blob has no such field and a decode of one runs off the end of the record rather than reporting a version |
