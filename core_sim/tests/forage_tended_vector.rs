...
fn the_published_per_biomass_rate_is_what_a_real_turn_credits_on_both_binding_sides() {
    // A trade-dominant crop, so the trade account clears the stockpile's integer rounding.
    for floor in [0.5_f32, 0.15] {
        for workers in [1_u32, FORAGE_WORKERS] {
            ...
            let _band = spawn_forager_with_workers(&mut app, tile, coord, floor, workers);
