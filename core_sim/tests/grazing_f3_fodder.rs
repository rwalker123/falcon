...
/// whole herd is GONE.** `fodder_delivery_rate` is the one F3 scratch field `ecological_carrying_capacity`
/// reads into `K` — added for EVERY herd with `fodder_per_biomass > 0`, penned or not. The original worry
/// was a *mobile escaped* herd carrying a stale hay rate and reading an inflated roam-range `K`. Under the
/// neglect-escape model (`docs/plan_fauna_neglect_escape.md` §2.4) there **is** no mobile escapee: a
/// fully-abandoned pen sheds its flock to the wild web over turns and, when the last animal is gone, the
/// empty managed entity is **despawned** — the stale rate goes with it, so the "inflated K on a mobile
/// herd" state is unreachable by construction. This fixture proves the despawn.
#[test]
fn a_lost_pen_drops_its_hay_delivery_rate() {
    /// A plausible sustained hay inflow a keeper's Fields would have stamped while tending the pen.
    const STALE_RATE: f32 = 20.0;

    // Seat a domesticated herd as a PEN on a REAL graze tile, stamp `stale_rate` hay as the labor arm
    // would for a kept pen, then leave it untended. Run `advance_husbandry` (Logistics shed pass) until
    // the flock bleeds out and the empty pen despawns.
    let mut app = base_world();
    let tile = pen_tile(&app);
    learn_foddering(&mut app);
