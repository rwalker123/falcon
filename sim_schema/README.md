...
| `src/world.rs` | the deliberately **flat** `WorldSnapshot`/`WorldDelta`, `SnapshotHeader`, `hash_snapshot`, `MapExport`, and the bincode/JSON codecs. **The bincode encoders are hash and fixture inputs, not a wire format** — `finalize`/`hash_snapshot` hash the bincode bytes, and `MapExport` rides JSON; the bincode snapshot *socket* was retired in #388, and these structs do not round-trip through bincode anyway (`skip_serializing_if` omits fields a non-self-describing format still expects back) |
