...
| `ui/hud/CommandFeedController.gd` | Owns the left-dock command feed card: the rolling entry list, signature de-duplication (`ingest_events`), client `note()`s, and the internal-scroll sizing. Plus a per-kind `KIND_STYLE` table (glyph + `HudStyle` colour): `narrative_beat` / `narrative_fork` render as **PROSE** — glyph + the line, gloss as the dim italic detail, **no `Turn N` prefix and no bold-label/italic-detail split** — while every other kind falls through to the original capitalize behaviour. Both glyphs are **line art** (`»` / `?`), not emoji (which blob at feed size) |
