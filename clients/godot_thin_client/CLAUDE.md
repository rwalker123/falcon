...
| `ui/MenuShell.gd` (`ui/MenuShell.tscn`) | The ONE shared menu surface (DRY) for BOTH the landing screen and the ESC pause menu; `mode` ("landing"\|"pause") re-filters a single registry-driven nav and re-lays-out (full-bleed vs centered card over a scrim). Built in code, styled through `HudStyle`. New Game pane = preset picker (earthlike / polar_contrast) + map-size picker (from `MapSizes.OPTIONS`, Standard default) + seed field. Functional items emit `new_game_requested`/`resume_requested`/`abandon_requested`/`exit_requested`/`apply_theme_requested`; Map Selection/Load/Save render inert placeholder panes. **The Options pane is live**: an "Interface scale" slider, a **Theme picker** over the four HUD palettes (with an always-visible caption stating whether the pick is on screen yet, plus an "Apply now" button shown only while it is not — applying re-installs the palette and reloads the scene, and in pause mode that rebuilds the world and ENDS the run, see `.claude/rules/client/sprites-widgets.md`), a "Fog of war" toggle row (`_make_toggle_row`) and "Map pan speed" + "Zoom speed" sliders, all persisting immediately via `ClientSettings`, plus an enabled "Restore defaults". It opens in BOTH landing and pause modes, so it works pre-run and in-run. For the Options rows' boundary rules see `.claude/rules/client/fog-of-war.md` and `.claude/rules/client/interface-scale.md` |
