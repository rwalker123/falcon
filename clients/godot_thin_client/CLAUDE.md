...
- **PER-WATER-TERRAIN shore profile (`shore_profile`)** — the five levers above are the GLOBAL profile, and it
  is tuned for OCEAN coasts, where its reaches are a small fraction of a huge body of water. An `inland_sea`
  (id 2) is typically a handful of hexes and the same profile swamps it — in particular the offshore **wisp**
  reads as noise across the middle of a lake. So a WATER terrain entry in `terrain_config.json` may carry an
  optional block that scales the profile of **its own** coastline:
  `{ "id": 2, "name": "inland_sea", …, "shore_profile": { "reach_scale": 1.0, "wisp_scale": 0.0 } }`
  - `reach_scale` multiplies **all four** reaches (`sand_band` / `foam_inland_band` / `foam_band` /
    `wisp_center_band`+`wisp_half_band`); `wisp_scale` multiplies the wisp's **half-width AND its strength**,
    so `0.0` cleanly removes the second disturbance without touching that water's sand or surf.
  - **A water terrain with no `shore_profile` gets the neutral default (1.0, 1.0)** —
    `SHORE_PROFILE_DEFAULT_REACH_SCALE` / `_WISP_SCALE` in `TerrainTextureManager` — which is a no-op, so
    deep_ocean / continental_shelf / coral_shelf / hydrothermal_vent_field render as they always have.
  - **Plumbing mirrors the per-layer mean-luminance table** (`layer_luma_texture`): `TerrainTextureManager`
    packs the profiles into `layer_shore_texture`, a **1×N FORMAT_RGF** image (R = reach_scale, G =
    wisp_scale, one texel per terrain id), bound once by MapView as the `layer_shore_map` uniform and fetched
    in-shader by layer index (`shore_profile(layer)`). `rebuild_layer_shore_map()` is public and **updates the
    ImageTexture in place** (so the binding survives) — that is how `blend_probe` sweeps profiles live.
  - **THE PROFILE IS KEYED ON THE WATER HEX'S TERRAIN, on BOTH sides of the waterline** (own layer when the
    fragment's hex is water, else the nearest qualifying water **neighbour**'s layer). This is a *correctness*
    requirement, not a style choice: every shore weight is one smoothstep of the signed coast coordinate `u`
    evaluated on both sides of the shared edge, so if the two sides picked different reaches the profile would
    be discontinuous **at the hex edge** — reintroducing exactly the hard line `u` exists to prevent. The water
    hex's terrain is also the only side both fragments can agree on (the land biome varies along a coast; the
    body of water does not) and the meaningful one ("lake or ocean?" is a property of the water).
  - **Known limitation:** a land hex bordering TWO different water bodies (e.g. a lake AND an ocean) switches
    profile at the nearest-edge bisector, which can show a faint seam *inland* between the two coastlines.
    Rare, and not worth a second blend to fix; noted here so it isn't rediscovered as a bug.
  - **Shipped:** only `inland_sea` carries a block today (`reach_scale 1.0` / `wisp_scale 0.0` — a placeholder
    while the lighter-lake variant is chosen). Per-**LAND**-biome shore gating (a grassy shore vs a wooded
    shore) is still deliberately NOT built — all coasts render the same beach+foam art. Verify via
    `tools/map_preview.gd` State Q (`_biome_band_terrain` carves an ocean bay so the ocean borders BOTH
    prairie and woodland) → `map_biome_blend.png` + `map_biome_shore_seam.png` (coast close-up), and the
    per-water-terrain profile via `blend_probe` **state 10 (L)** below.
