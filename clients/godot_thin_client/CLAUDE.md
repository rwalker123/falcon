...
| `extraction-workings.md` | The workings on the ground: the tile card's per-material rows, the two compose sheets, the deposit ladder, the `Workings` pool and its roster | `hud_deposit_vocab.gd`, `dict/deposits.rs`, `ui_preview/chapters/workings.gd` |
