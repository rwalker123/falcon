...
- **NAVIGABLE-CHANNEL pass (shader), right AFTER the Minor/Major pass** (so a Major feeding a navigable
  ...
  - **`blend_class` is `"flat"`, not `"water"`** (a *render* eligibility class with no sim meaning — the
    ...
    **The bank ALSO carries a per-terrain `blend_profile`** (`2.6 / 2.2 / 2.6` — see Edge Blending), and it
    has to: with the global ecotone alone the flat↔flat blend fires correctly but is far too narrow and too
    straight for the bank's contrast against its neighbours, and the corridor renders as a **chain of grey
    hexagons** rather than a river valley. **The bank's gravel art is graded to luma ~89 deliberately** — it
    sits *between* prairie/scrub (112–127) and floodplain/alluvial (55–58), the two neighbour families a
    corridor actually touches — so this is fixed in the **shader/config, never by re-grading the texture**.
    Judge it on `blend_probe` state **17 (BANK)**, which renders both neighbour families in one frame.
  - The shore pass additionally **skips any edge with a navigable hex on EITHER side**: `blend_class`
    ...
    drains into. A river meeting the sea is not a coast. **This must be an EDGE-LEVEL exclusion, not a
    `shore_profile` entry** (see Shoreline → per-water-terrain shore profile): the profile field is keyed on
    the **water** side — only a `CLASS_WATER` hex contributes one — and a navigable hex is land-class by
    construction, so it can never supply a profile. The profile that would apply at the mouth is the **sea's**,
    and that sea must keep it for its coast everywhere else. Dropping the edge removes the whole chain at once
    (profile, waterline cross-fade, sand, surf, wisp all live under the pass's `best_d` guard) and does so
    symmetrically from **both** hexes' frames, so no half-drawn coast can survive on one side of it.
