...
| `ui/hud/DisclosureController.gd` | `RefCounted` controller (HUD decomposition) owning the **detail-row disclosure cluster** — the NODE half of the detail-render layer: the per-render caret state (`state()` → `DetailFormat.Context.disclosures`), the stashed breakdown payloads, the shared breakdown `PopupPanel`, and the `meta_clicked` dispatch. **The cut with `DetailFormat` is along NODE OWNERSHIP, not "formatter vs popover"** — the two are bidirectionally coupled (the formatter emits the `[url]` meta only this file parses; the popover renders THROUGH the formatter), so what separates them is that this half must `add_child` a Window, which a static module cannot and a `RefCounted` cannot either. Hence `setup(popover_host: Node, refresh_hosts: Callable)`: the HUD CanvasLayer is the host (the `TurnOrbController` pattern), and `refresh_hosts` collapses the old `_render_band_into_panel` + `_render_subject_drawer` pair into **ONE injected Callable** — the single inbound re-render edge, since WHICH hosts can show a caret is `HudLayer`'s knowledge. Surface: `setup` · `wire_label(label)` (replaces BOTH `meta_clicked` bind sites — the drawer's long-lived `%OccupantDetail` and the Band panel's per-render vitals label, each binding ITSELF as the popover's anchor) · `state()` · `clear_rows()` · `register(row_label, kind, band, lines)` · `food_breakdown_lines(band)`. Hud holds it as `_disclosures`, constructed in `_ready` after `_drawercompose`. **The breakdown opens in a POPOVER, never inline, and that is a correctness rule** — inline growth of the vitals label happened AFTER the Band panel had picked its zone height tier, and the `clip_contents` zone hosts then sliced the WORKFORCE row and ate both role cards |
