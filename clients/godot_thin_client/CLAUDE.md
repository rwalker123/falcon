...
| `GameLaunch.gd` (autoload) | Cross-scene handoff: `pending_new_game` dict set by LandingScreen, consumed + cleared by `Main._build_new_game_command` — which also records what it RESOLVED to `active_new_game`, so a scene reload rebuilds the same world instead of the dev default. Also owns `apply_theme_now()`, the in-process rebuild behind the Options pane's "Apply now" (`.claude/rules/client/sprites-widgets.md`) |
