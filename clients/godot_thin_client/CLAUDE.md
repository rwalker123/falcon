...
| `ui/LandingScreen.gd` (`ui/LandingScreen.tscn`) | The boot main-scene (`project.godot` run/main_scene): a MenuShell in landing mode over a dark ground. `new_game_requested` stashes params in `GameLaunch.pending_new_game` and swaps to `Main.tscn`; `exit_requested` quits. Also **claims the player's window** — `project.godot` boots windowed + `no_focus` so the `tools/` render harnesses never steal the keyboard, and this scene (which only the game instantiates) promotes it to focused fullscreen; see `.claude/rules/client/test-harnesses.md` → "The tool window is quiet by DEFAULT" |
