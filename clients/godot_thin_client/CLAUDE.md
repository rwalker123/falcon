...
- **THE TRAILING CHROME RAIL — the HUD's bottom-bar chrome SHARES a horizontal dock's row** (issue #324).
  ...
  - **ONE column, not a gutter at each end.** A leading + trailing pair was built first and rejected on
    ...
    `max(nav, turn)` ≈ 302 — measured 296 on the default Standard map — which hands the zones ~260px
    back (work zone 688 → 948 at 1920×1080) and drops the wide→narrow flip from ~1618px of window width
    to **~1352px** (1056 + the rail; across the whole `MapSizes.OPTIONS` roster, ~1352–1364). **The
  ...
  - **`_rail_width()` is folded in at exactly TWO places** — `_shell_is_wide()` (the threshold is
...
- Verify chrome + reflow via `tools/band_panel_preview.gd`
  ...
  **The five `band_panel_dockrow_*` states are the DOCK-ROW REFLOW's frames** (issue #324), rendered at
  ...
  `_assert_no_rail_width`, and `_assert_chrome_home_exact`. **The shell-threshold probes derive their widths
