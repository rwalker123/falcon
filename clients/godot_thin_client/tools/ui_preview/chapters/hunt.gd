...
## The last work-party ask the far-herd state's recording sender saw — so the claim that the section
...
## playtest crew.
const FAR_PARTY_HUNTERS := 2
## The fixture's authored caravan reply, in the reply's own fields, and **every figure is one the game
## can produce**: 8 hexes out with an apron of 2 is a 6-tile walk, and at the shipped
## `band_move_tiles_per_turn` of 1 that is 6 turns each way.
const FAR_PARTY_WALK_TILES := 6
const FAR_PARTY_WALK_TURNS := 6
## A MEAN over the horizon, and deliberately not a whole number, so the section's rounding is
...
## The FAR herd's sheet — a herd past the band's apron. It is recorded so its grammar can be held to
## the local hunt's: the far sheet is the same sheet plus a party section, never a second control order.
const COMPOSE_SPINE_KEY_FAR_HUNT := "far hunt"
...
## `8 − 2 = 6` each way — at the shipped MOVE RATE of 1, so the walk is 6 turns each way.
func _far_boar_band() -> Dictionary:
...
## The Wild Boar the far-herd state composes against, carrying the caravan reply the fixture's
## answerer hands back (`ForecastFx.WORK_PARTY_FORECAST_KEY`).
func _far_boar_herd() -> Dictionary:
...
func run(harness) -> void:
	...
	var far_band := _far_boar_band()
	h._hud._band_labor._player_bands = [far_band]
	h._hud._band_labor._player_band = far_band
	h._hud._compose.reset_hunt_source()
	h._hud._compose.set_hunt_band(-1)
	var far_boar := _far_boar_herd()
	_last_work_party_ask = {}
	var far_query: ForecastQuery = h._hud.forecast_query()
	far_query.set_sender(func(request_id: int, ask: Dictionary) -> bool:
		if String(ask.get("kind", "")) == ForecastQuery.KIND_WORK_PARTY:
			_last_work_party_ask = ask.duplicate()
		far_query.deliver.call_deferred([ForecastFx.answer(h._hud, request_id, ask)])
		return true)
	h._show_herd(far_boar)
	h._compose_herd(far_boar, FAR_PARTY_HUNTERS, SourceForecast.FLOOR_FOOD_PEAK)
	await h._settle()
	await h._save("herd_hunt_far_party")
	h._assert_compose_sheet_fits("herd_hunt_far_party")
	var far_sheet: Control = h._hud._drawercompose._compose_sheet
	# **THE ORDINARY HUNT'S SPINE, NOT A SEPARATE GRAMMAR** — the retired expedition sheet recorded
	# a spine of its own; the far herd's sheet must read exactly the local hunt's, band → floor →
	# crew → kit → improvement.
	h._record_compose_spine(COMPOSE_SPINE_KEY_FAR_HUNT)
	# ⛔ **THE COMMIT IS THE LOCAL HUNT'S VERB**, and the expedition sheet's faces are gone. The commit
	# is found by identity (`COMPOSE_COMMIT_META`), so the claim is about the button the press sends.
	var far_commit := Q.compose_commit_button(far_sheet)
	h._assert_hud("a herd past the apron commits as an ordinary hunt (got \"%s\")"
			% ("" if far_commit == null else far_commit.text),
		far_commit != null and far_commit.text == HudComposeVocab.ASSIGN_LOCAL_HUNT_BUTTON
			and not far_commit.disabled)
	h._assert_hud("…and the sheet carries none of the retired expedition sheet's words",
		not Q.has_label_containing(far_sheet, RETIRED_EXPEDITION_HINT_NEEDLE)
			and not Q.has_label_containing(far_sheet, RETIRED_TRIP_VERDICT_NEEDLE))
	# **THE SECTION, BY EQUALITY, LINE BY LINE, through the shipped formats and the fixture's own
	# numbers.** A presence test passes on a section that says anything; the four facts each have one
	# right spelling here.
	...
	# row will print the turn after commit, so the sheet promises what the row shows. It is the
	# section's `rate_home`, which is why the section no longer states it a second time.
	var far_food := Readout.yields_account_number(far_sheet, SourceForecast.YIELD_ACCOUNT_FOOD)
	...
	# ⛔ **AND THE CAPTION OVER IT DESCRIBES THAT NUMBER.** `next turn` would be false: next turn a new
	# posting is still walking out and delivers nothing. No likely-take suffix — the rate home is the
	# caravan forecast's mean, not a point of the curve's band.
	h._assert_hud("…under a caption naming the rate home, not next turn (got \"%s\")"
			% Readout.yields_header(far_sheet),
		Readout.yields_header(far_sheet) == HudComposeVocab.YIELD_HEADER_HOME_RATE.to_upper())
	...
	h._assert_hud("…priced at the crew the stepper composed (%d)" % FAR_PARTY_HUNTERS,
		int(_last_work_party_ask.get("workers", -1)) == FAR_PARTY_HUNTERS
	...
	var road_lines := DrawerComposeController.work_party_section_lines({
		...
		HudComposeVocab.HUNT_CREW_LABEL)
	h._assert_hud("a road covering the run states no walk, and the rest of the copy's edges hold — got %s"
			% str(road_lines),
		road_lines.size() == 3
			and road_lines[0] == HudComposeVocab.WORK_PARTY_NO_WALK
			and road_lines[1] == HudComposeVocab.WORK_PARTY_ON_ROAD_RARELY
			and road_lines[2] == HudComposeVocab.WORK_PARTY_FIRST_LOAD_FORMAT
	...
	var never_lines := DrawerComposeController.work_party_section_lines({
		...
		HudComposeVocab.HUNT_CREW_LABEL)
	h._assert_hud("…and a one-tile walk, a plural road and no load in the horizon read as English — got %s"
			% str(never_lines),
		never_lines.size() == 3
			and never_lines[0] == HudComposeVocab.WORK_PARTY_WALK_FORMAT % [
				HudComposeVocab.WORK_PARTY_TILES_ONE, HudComposeVocab.WORK_PARTY_TURNS_ONE, 1]
			and never_lines[1] == HudComposeVocab.WORK_PARTY_ON_ROAD_FORMAT % [2, "hunters"]
			and never_lines[2] == HudComposeVocab.WORK_PARTY_NO_FIRST_LOAD)
	...
	# the herd (inside its apron) → an ordinary hunt with NO party section. The band-picker selection
	...
	# **INSIDE THE APRON THE CAPTION IS THE LOCAL ONE** — the identity half of the caption claim.
	var near_caption := Readout.yields_header(h._hud._drawercompose._compose_sheet)
	h._assert_hud("inside the apron the yields caption still reads `next turn` (got \"%s\")" % near_caption,
		near_caption.begins_with(SourceForecast.YIELD_ROW_HEADER.to_upper())
			and near_caption != HudComposeVocab.YIELD_HEADER_HOME_RATE.to_upper())
	# **A SOURCE INSIDE THE APRON GROWS NO SECTION** — the identity half of the work party's pair.
	h._assert_hud("a herd inside the picked band's apron shows no work-party section",
		Q.find_meta_node(h._hud._drawercompose._compose_sheet,
			HudWidgets.WORK_PARTY_SECTION_META) == null)

	# State 3j — same two bands, FAR band selected via the picker (entity 812, ~27 tiles away): the SAME
	# herd is measured past THIS band's apron, so the sheet asks the caravan forecast — and the
	# fixture herd authors no reply, i.e. the sim's `posts_a_party: false`, so no section mounts.
	h._hud._compose.set_hunt_band(int(BandFx.hunt_distance_bands()[1]["entity"]))   # FAR band
	h._compose_herd(HerdFx.hunt_distance_herd())
	await h._settle()
	await h._save("herd_hunt_band_far")
	var band_far_sheet: Control = h._hud._drawercompose._compose_sheet
	# ⛔ **INSIDE THE APRON — AND WHERE THE SIM POSTS NO PARTY — THE HEADLINE IS UNTOUCHED.** The
	# substitution is the identity on an empty view and on a reply posting no party; driven on the
	# producer, because a sheet whose headline was rewritten to the same number renders the same frame.
	var identity_model := {DrawerComposeController.YIELD_MODEL_ROWS: [
		{SourceForecast.YIELD_ROW_ACCOUNT: SourceForecast.YIELD_ACCOUNT_FOOD,
			SourceForecast.YIELD_ROW_VALUE: 0.5, SourceForecast.YIELD_ROW_AFTER: 0.2}]}
	h._assert_hud("inside the apron the sheet's food headline is left exactly as the source states it",
		h._hud._drawercompose._with_home_rate(identity_model, {}) == identity_model
			and h._hud._drawercompose._with_home_rate(identity_model, {
				"state": ForecastQuery.STATE_READY, "answer": {"posts_a_party": false,
					"rate_home": FAR_PARTY_RATE_HOME}}) == identity_model)
	h._assert_hud("where the sim answers no party, a far source mounts no section either",
		Q.find_meta_node(band_far_sheet, HudWidgets.WORK_PARTY_SECTION_META) == null)
	h._assert_hud("…and it is still the ordinary hunt's commit, whatever the distance",
		Q.compose_commit_button(band_far_sheet) != null
			and Q.compose_commit_button(band_far_sheet).text == HudComposeVocab.ASSIGN_LOCAL_HUNT_BUTTON)
	...
	# States 3n–3o — the LOCAL hunt sheet (herd inside the band's apron). The preview line reads the
	...
	# ⛔ **AND THE FAR HERD'S SHEET READS IN THE LOCAL HUNT'S ORDER** — the retired expedition sheet
	# had a grammar of its own; the work party adds a section and changes nothing else.
	var far_spine: Array = _spine_grammar(h._compose_spines.get(COMPOSE_SPINE_KEY_FAR_HUNT, []))
	var local_spine: Array = _spine_grammar(h._compose_spines.get(COMPOSE_SPINE_KEY_HUNT, []))
	h._assert_hud("the far herd's sheet reads in the local hunt's control order — far %s, local %s"
			% [str(far_spine), str(local_spine)],
		not far_spine.is_empty() and far_spine == local_spine)
	...
	# ---- A SHEET COMPOSES FOR THE BAND THE PLAYER IS LOOKING AT, AND ITS NAME IS THE BAND'S -----------
	await _compose_band_states()
...
# ---- A WILD AUROCHS'S OWN TERMS ------------------------------------------------------------------
## Shared by the sub-one, spear-short and retreat frames below. They were first written for the
## retired empty-raid refusal (a thriving herd a party of one could not kill); the herd outlived it.
## `B/K` a hair over 0.9, ten of eleven animals, so `body_mass` and the pair agree by construction.
...
## A clean binary rate, so the sheet's `1 / engageRate` inversion lands on a whole number.
...
## The herd's two defensive axes. `durability / (attack − defense)` = `160 / (20 − 5)` ≈ 10.7
## hunter-turns for ONE hunter — a winnable, slow fight.
const AUROCHS_DEFENSE := 5.0

const AUROCHS_DURABILITY := 160.0

func _compose_band_states() -> void:
