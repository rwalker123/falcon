...
##
## ⛔ **THE FOUR STATES ARE THE §7 FORK AND THE TWO-WORKINGS KEY, and neither can be shown by one
## frame.** `regrowth_rate > 0` decides which sentence a working publishes — the over-cut pair against
## the runway — and a flint scatter and a quarry are both `extraction`, so a chapter that staged one
## renewing and one finite working would pass on a client forking on `branch`. And a tile holds up to
## TWO workings on one `(tile, material)` key each, which is the one claim a single-block card cannot
## fail.
##
...
## **THE OVER-CUT PAIR.** `sustainable_take` is the growth term itself; `actual_take` is what the
## crews paid out last turn. The wood seam is being cut HARDER than it renews, which is the state §7's
## renewable warning exists for.
...
func run(harness) -> void:
	...
	# one does rather than on a tile carrying two workings.
	h._show_tile(_workings_tile([]))
	await h._settle()
	h._assert_hud("a hex nobody has worked offers no workings action at all",
