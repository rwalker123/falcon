...
func _ready() -> void:
	...
	# The two-line button face's STATE COLOUR (issue #383) — offscreen, so it adds no frame and the
	# bit-identity claim above is untouched.
	await _assert_two_line_face_states()
...
func _assert_two_line_face_states() -> void:
	...
	for spec in [
		...
		var pill := _find_crew_target(targets, String(spec[0]))
		...
		_assert_hud("%s renders a two-line face to colour" % what,
	...
	for spec in [
		...
		_assert_hud("%s draws live ink on both lines while ENABLED (%.2f / %.2f)"
				% [what, lit[0], lit[1]],
			lit[0] >= TWO_LINE_FACE_MIN_INK and lit[1] >= TWO_LINE_FACE_MIN_INK)
		_assert_hud("%s: line 1 goes DIM with its button (%.2f → %.2f)" % [what, lit[0], dim[0]],
			lit[0] - dim[0] >= TWO_LINE_FACE_MIN_DIM)
		_assert_hud("%s: line 2 goes dim TOO — not left bright beside a faded box (%.2f → %.2f)"
				% [what, lit[1], dim[1]],
