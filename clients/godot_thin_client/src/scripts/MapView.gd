	...
	for entry_variant in population_variant:
		...
		var marker := {
			...
			"max_expedition_party_size": int(entry.get("max_expedition_party_size", 0)),
				# Pre-launch hunt-trip forecast levers (global expedition/labor config echoed on every
				# cohort). The HUD combines them with the target herd's `hunt_policy_ceilings` to tell the
				# player how many turns a party needs to fill BEFORE the herd click commits — so they must
				# ride the marker (the targeting flow carries a copy of the band dict).
