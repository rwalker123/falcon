...
# row — and `ASSIGNMENT_WALK_TILES_KEY == 0` is NOT that test: a road covering the whole run takes a
# real posting's walk to zero.
const ASSIGNMENT_PARTY_X_KEY := "party_x"
const ASSIGNMENT_PARTY_Y_KEY := "party_y"
const ASSIGNMENT_PARTY_WORKERS_KEY := "party_workers"
# **LIVE, THIS TURN** — the caravan's hunters out with a pack or walking back to rejoin. It moves
# turn to turn (`0, 1, 1, 0, 2…`) and that is honest: nobody is simulated as a unit, one hunter
# leaves each time the take fills a pack.
const ASSIGNMENT_HUNTERS_ON_THE_ROAD_KEY := "hunters_on_the_road"
# The one-way walk, measured from the band's APRON and shortened by any road. A fact about where the
# posting is; it does not count down.
const ASSIGNMENT_WALK_TILES_KEY := "walk_tiles"
# ⛔ **`> 0` ONLY WHILE THE WHOLE PARTY IS STILL WALKING OUT, AND `0` FOR THE REST OF THE POSTING.**
# The walk out happens once and is never re-raised, so that zero drops the walking-out line outright.
const ASSIGNMENT_WALK_OUT_REMAINING_KEY := "walk_out_remaining"
...
# `huntCrews[0]` sim-side, so the gate composed from it answers for the best-armed run alone — the
# reassuring half of a split party. **Nothing states the other half any more**: `hunt_crew_split_model`
# was what did, and it is retired below.
const BAND_HUNTER_ATTACK_KEY := "hunter_attack"
const HERD_DEFENSE_KEY := "defense"
const HERD_DURABILITY_KEY := "durability"

# ⛔ **RETIRED: `hunt_crew_split_model` AND ITS WHOLE VOCABULARY** — `HUNT_CREW_SPLIT_FORMAT`
# (*"%s%d of your %d hunters can take %s; the other %d %s and land nothing on it at any headcount."*),
# its two tails `HUNT_CREW_SPLIT_BARE_CLAUSE` / `HUNT_CREW_SPLIT_UNDER_CLAUSE`, and the
# `HUNT_CREW_PARTY_UNSET` sentinel that told a host with no stepper from one with a party of zero.
#
# It walked `huntCrews` as a prefix of the composed party and said how many of the hunters being SENT
# could clear this quarry's defence at all. Reported from play, stacked under
# `KitRoster.shortfall_line`'s *"1 of 15 Stalking kits available"* on one ASSIGN HUNTERS sheet:
# *"The yellow message mentions the hunters. I'm not sure we need that message at all, we have the
# stalking kit message, it seems the second is redundant, you can remove it."*
#
# ⛔ **WHAT WENT WITH IT IS NOT ONLY WORDING, and `labor-ui.md` → "RETIRED — the hunt crew-split
# sentence" is where that is written down.** The surviving kit line says the GEAR is short; this one
# said the under-geared share of the party lands **nothing at any headcount** against *this* quarry —
# a different outcome from working at the bare-handed tier, and against a defended animal the
# difference is the whole hunt. No sentence states it now.
#
# **The gate's own refusal is untouched** (`hunt_gate_model_at` above): a fight NOBODY can make still
# says so, in DANGER. Only the cleared-gate complement is gone.
#
# `DetailFormat.band_hunt_crews` / `band_hunt_headcount` and the three `HUNT_CREW_*_KEY` constants
# were this model's only readers and have none now; `HUNT_CREW_WORKER_EPSILON` keeps a live one in
# `DetailFormat.kit_coverage`.
...
## and every other key reads 0 with it — so a reader testing `walk_tiles`, `hunters_on_the_road`
## or `net_rate_home` instead would draw a party block on a local row the turn one of those is honestly
...
static func party_readout(row: Dictionary) -> Dictionary:
    ...
    return {
        ...
        ASSIGNMENT_HUNTERS_ON_THE_ROAD_KEY: int(row.get(ASSIGNMENT_HUNTERS_ON_THE_ROAD_KEY, 0)),
