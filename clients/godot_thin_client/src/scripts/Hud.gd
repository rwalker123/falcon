...
func set_band_city_panel(panel: BandCityPanel) -> void:
    ...
    # The dockable panel's Food/Morale labels are the same click-to-expand disclosures as the drawer.
    if _band_city_panel != null:
        var detail_label: RichTextLabel = _band_city_panel.get_band_detail_label()
        if detail_label != null and not detail_label.meta_clicked.is_connected(_on_detail_meta_clicked.bind(true)):
