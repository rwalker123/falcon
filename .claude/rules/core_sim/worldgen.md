...
## ⛔ A SPAWN STOCKS NOTHING — NO EQUIPMENT AND NO MATERIAL

`spawn_population_entity` inserts `BandEquipment::start_stocked_owned(...)`, which at the shipped
`start_stock_fraction: 0.0` stocks **nothing** (`equipment.md` → "A SPAWNING BAND OWNS NOTHING"), and
a plain `LocalStore::new()` beside it.

Both halves were once generous and both were retired deliberately. The material half briefly seeded a
per-worker pile of `wood` and `stone` from a `MaterialDef::start_stock` block, through a
`start_stocked_materials` helper here; **the block, the helper and the whole mechanism are gone**
(`crafting.md` → "NOTHING IS STOCKED AT SPAWN"). What a band opens with is what the **player**
allocated in the turn-one loadout window, after this map was on screen
(`campaign.md` → "The opening loadout").

- **`party_workers(size, working_fraction)`** — `(size × working_fraction).floor()` — is what remains
  of the sizing, and it is now a **shared** helper rather than an expression: the equipment stock and
  the loadout's **kit budget** (one kit per working-age hand) are both measured against it, so
  *"a party's worth"* means one thing in this crate rather than one thing per call site.
- **The brackets and the larder are still seeded at Startup** by `apply_starting_inventory_effects`,
  which needs the demographic split this does not; that comment stays true.
