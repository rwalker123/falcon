...
|---|---|---|
| **`spears`** | `attack` **20** (equipped) | per **landed blow** |
| **`sled`** — travois, drag harness | `hunt_carry` **40** (equipped, on its **tier**); the sledless **12** is `labor_config`'s baseline. **A PEN IS COLLECTED ON THIS SAME STAT** — see "Carry is carry" | per **biomass hauled home from a hunt**, **and** per **biomass BUTCHERED off a pen** — the one item on two quanta |
...
| **`crook`** — a long bone staff, plus the halters and butchering stone a keeper works a beast with | `build_work` **+0.5 per worker per turn** on branch **`animal`** (equipped, on its **tier**), and `upkeep_work` beside it | per **unit of build progress accrued**, **and** per **unit of keeping work supplied** |
...
| **`wayfinding`** — tallies, marked staves, a fire-drill | **TWO** sight stats, both unequipped: `scout_vantage_range` **1** (the posted vantage's bare reach; the equipped `2` is `labor_config`'s) and `expedition_sight_range` **6** (a detached party's; the equipped `9` is `expedition_config`'s `observe_sight_range`) — see "Two sight stats, one item" | per **tile revealed for the FIRST time**, from a posted vantage **or** from a ranging party's march |
...
| **`billet`** — an antler billet; bounds `stone` | `craft_speed` **1.8**, ceiling **0.88**, efficiency **0.95** — the *highest* of the four, because stone is the least scarce thing a bench works | likewise |
...
> goes to the next field, so it is kit: it wears with use and its absence makes the work *dearer*,
> never impossible. A fence **stays in the ground**, so it is material: it is spent, and no number of
> hands replaces it. So `hurdles` are a `materials.json` row the `animal:pen` rung eats on **both** its
> build pile and its upkeep rate, and the item is gone.
>
...
> through a `shares_equipped_rate_with` link, so this put one stat's two sides on one item, and the
> `biomass_collected` quantum went with it (**appended**, so the sled's headline stays
> `biomass_hauled` and its gauge reads exactly what it read).
>
> ⛔ **THAT WAS THE MOVE THAT KILLED THE STAT.** *"A band that corrals a herd and leaves the
> assignment on `big_game` collects at 12 rather than 40"* — the mistake this roster was built to
> punish — stopped being a mistake the moment both sides sat on the sled: every kit carrying one
> collected a pen at the equipped rate, so `big_game` and `trapping` read alike there and the only
> thing `pen_carry` still separated was the *sledless* crew — which `hunt_carry` separates too, at the
> same two numbers. Two names for one number, and the deletion is recorded under **"Carry is carry"**
> below. `EquipmentConfig::kit_supplying(Hunt, HuntCarry)` — which a corralled herd's `defaultKitId`
> resolves through — answers **`big_game`**, the earliest hunt kit in file order carrying a sled.
> That left the `husbandry` kit strictly dominated by `big_game`, and §4.9 item 12b deleted it rather
> than finding it something of its own (see above).
>
...
> not for an existing one — the retired `hurdles` led with `biomass_collected` and read *"≈2500
...
> **The four tools are in NO kit, and `validate` enforces that.** A tool serves the *bench*, not a
...
`WearQuantum` gained `biomass_collected` and `tile_revealed` (and, briefly, `fight` — retired into
`strike`, below). Still no `turn` variant — that is `docs/plan_denial_raid.md` §1.2 enforced by the
type — and no JOB's gear is charged on another job's use, pinned by
`each_kit_wears_on_a_use_quantum_of_its_own_job`. **Two weapons DO share `strike`**, and the kit mask
is what keeps a hunt from blunting the camp's clubs; that test asserts it directly.

- **`tile_revealed` means FIRST-EVER revealed, and it has TWO charge sites.** A parked band re-sees
  the same ring every turn and its own centre reveals new ground whenever it walks, so charging per
  tile *seen* — or, for a resident band, for any reveal rather than a **posted vantage's** — would be
  a turn clock in a per-use costume. The second site is a **detached party's march**
  (`expeditions.md` → "A party's reach is its KIT's"), which is the same statement one observer over:
  it is charged at observe time, and only for tiles neither already in its own buffer nor already on
  its faction map. `FactionVisibilityMap::mark_active` returns whether it lifted an `Unexplored` tile, because
  ...
  Pinned from both ends — `wear_is_charged_for_work_not_for_turns_elapsed` asserts the gear really
...
- ~~**`fight` is one ENGAGEMENT, not one casualty inflicted.**~~ **Retired into `strike`** — see
  "Wear follows the work actually done". Its reasoning survives one level down: pricing gear on the
...
### Carry is carry — a pen and a range are collected on ONE rate

**There is exactly one carry stat per food web, and `EquipmentStat::PenCarry` was deleted to make
that true** (issue #543). The rule the issue states:
...
> kit, and completely blind to what it is standing on.*

So `advance_labor_allocation`'s Hunt arm resolves `hunt_per_worker_biomass` **once**, and a corralled
herd and a wild one read the same local. The `is_corralled()` fork that used to choose between two
rates is gone from the sim arm and from the assign-time seed (`seed_source_yield`) alike — the seed
and the resolved row must stay exactly-equal or `yield-forecast.md`'s forecast-equals-actual
invariant is broken on the one surface a player commits from.

> #### ⛔ THE SPLIT WAS REAL ONCE, AND THE DEAD CLAIM IS QUOTED HERE SO IT IS NOT RE-DERIVED
>
> This section used to read **"A pen is collected on `pen_carry`, and bringing a sled to it costs
> you"**, and argued: *"`PenCarry` is a separate stat from `HuntCarry`, and that is the physical claim
> 'one item, one job' already makes twice. A sled drags a carcass in off the range; a pen stands at the
> camp, and what bounds a slaughter there is handling gear… a band that corrals a herd and leaves the
> assignment on `big_game` is working its pen with a drag harness and no handling gear, and collects at
> 12 rather than 40."*
>
> **It was true while `hurdles` were an ITEM.** The stat's *unequipped* side lived on them, so the
> two kits genuinely priced a pen differently — a real distinction between two tools. `hurdles` became
> a **MATERIAL** (`docs/plan_standing_upkeep.md` §4.9 item 12, see "`hurdles` LEFT THE ROSTER"), the
> item was deleted, and **both** sides of the pair landed on the **sled**, which already owned the
> equipped side. **The discriminating item was gone; only the two names survived it.**
>
> **What the deletion actually moved: nothing.** For every shipped kit the two stats already answered
> the same number — a crew with a sled read `40` on both (`pen_carry`'s `Unequipped` arm resolved the
> equipped side off `hunt_carry`'s tier), a crew without one read `12` on both (the sled's item-level
> `Unequipped(12)` against `labor_config.hunt.per_worker_biomass_capacity`, which is the same `12`) —
> and both call sites passed the same `baseline_haul_rate`.
>
> ⛔ **ONE THING DID MOVE, AND IT IS AN IMPROVEMENT RATHER THAN A RETUNE.** `pen_carry` had no
> **grade** declaration, so its equipped side resolved through `equipped_reference` to the item
> table's *default tier* — a flat `40` whatever a crafted sled was actually made at. `hunt_carry` is
> graded (`recipes.json`: poor 30 / fair 34 / good 40 / excellent 46). So a band working a pen with a
> **poor** sled now collects `30` where it collected `40`, and with an **excellent** one `46`. A
...
- **`fauna::herd_default_hunt_kit`** uses it to pick the kit the compose sheet **opens on**. A pen has
  ...
  not the sim deciding carry — whichever kit they send is resolved through the one band-wide rate.
...
`docs/plan_standing_upkeep.md` **§4.8**). Neutral at **`0.0`**; **`plain` hoes are +0.5 on the `plant`
branch and the `plain` crook +0.5 on `animal`** (the knapped `flint` hoes are +0.7), so an equipped worker banks `PER_WORKER_OUTPUT + 0.5 =
...
**The defect it closes.** The animal-handling gear only paid off *once a pen existed* — its other
stat was read on a corralled herd and nowhere else — so the gear did nothing across the entire climb
that produces the pen. That is backwards for what the gear physically is: halters, a staff and a
butchering stone are **animal-handling** tools, and `Tame` and `Corral` are exactly the turns a band
spends handling animals. (The item that carried both stats has since split: the pen's collection rate
went to the **sled** and `build_work` to the **crook** — see "`hurdles` LEFT THE ROSTER" — and the
collection rate was then deleted outright, see "Carry is carry". The defect and its fix are unmoved.)

- **NOT a two-tier rate, and that is what lets a second item declare it.** The `TWO_TIER` stats
  find their other side by searching the whole item table and taking the first match, so each may be
  declared by exactly one item or the answer resolves alphabetically. `hoes` are a second declarer,
  and legal by construction. So the stat is the kind a kit resolves as the **max of what
  its live items declare** (`KitChoice::best_declared` — `dispersion` and `exposure`'s shape), and a
  spent item stops contributing it. **The neutral it steps back to is `0.0`, not `1.0`** — the two
  live on `EquipmentStat::neutral`, so reading one as the other is not something a call site can do.
- **NOT named for husbandry, and the WEB IS NOW A QUALIFIER ON THE EFFECT.** Both webs' rungs read
  the one build seam, so a stat keyed to the animal branch would have needed renaming the day a hoe
  shipped. What is animal-only is the *content*, and it says so: the `crook` declares
  `"branch": "animal"` and `hoes` declare `"branch": "plant"`, and outside its branch an effect
  contributes the neutral **`0.0`** (`NO_BUILD_GEAR`). Reading that neutral as `1.0` would hand every
  ungeared crew a free work unit per worker off every job, which is exactly the wrong-neutral
  confusion `EquipmentStat::neutral` exists to make impossible.

  > #### ⛔ A `build_work` EFFECT **MUST** DECLARE ITS `branch`, AND NOTHING ELSE MAY DECLARE ONE
  >
  ...
  > *when it applies*. `branch` is that, for builds, and it is resolved the same way: there is no
  > *"this tool is not for that"* refusal anywhere, only a term that resolves to the neutral.
  >
  > Values are the ladder's own (`RungBranch`: `"plant"` / `"animal"`). `validate` rejects an
  > **absent** branch on a `build_work` effect — an unqualified tool would apply to *both* webs
  > silently, `config-loading.md`'s "looks live but isn't" in its most reassuring direction — and a
  > **present** one on any other stat, the twin of the mass-bound rule, since nothing else is
  > resolved against a web. Pinned by `validate_rejects_a_build_work_effect_with_no_branch` and
  > `validate_rejects_a_branch_on_a_stat_no_build_resolves`, with the resolution itself pinned in
  > both directions **and with the bundle** by
  > `a_build_tool_serves_its_own_web_and_two_of_them_do_not_compound`.
- **IT IS COVERAGE-WEIGHTED, like the carries — and THIS ANSWERS THE QUESTION `BuildRate` LEFT
  ...
  faster* — measured, one set of hurdles among ten keepers resolved **×1.05** against the ×1.5
  declared, and every un-geared hand added made the build *slower*. A per-worker contribution off the
  **cost** is a **SUM**, not an average: an un-geared hand adds zero rather than diluting.
  - **`t = Σ over the crew`, and `KitCoverage::weighted_rate × head count` is exactly that sum** —
    the partly-equipped-party seam the carries already run on. Five hoed workers and five bare answer
    `5 × worth`, the same shape as five sledded hunters and five sledless hauling `5 × 40 + 5 × 12`.
  - **The tool is WIELDED.** A worker holding one contributes its worth; a worker without one does
    not. `labor.rs` resolves it as
    `crew_coverage.weighted_rate(|kit| equipment_cfg.build_work_per_worker(kit, &band_kit))`, and the
    labor arm sums it with `intensification::build_work_from_gear`.
  - **A start-stocked band holds ONE set of hurdles**, so ten keepers take *one worker's worth* off
    the job — pinned by
    `labor::labor_yield_tests::the_handling_kit_takes_work_off_the_job_rather_than_speeding_the_crew`,
    which also asserts the two arms' per-turn **accrual is identical**. A half-geared build is
...
- **The stored companion cost stays the RAW job, and the meter JUMPS on completion.** The bar the
  ...
  effective bar sets the meter to the raw cost (`forage::banked_or_paid_off`) — the jump is the units
  the tool pre-paid, and it is honest: they were worked, they were simply worked by the tool.
- **Evaluated live each turn** off the crew's current kit, so gaining or wearing out a tool mid-build
  takes effect immediately and **no state records "this build was geared"**. What is published is a
  per-turn scratch reading (`buildWorkFromGear`), not a stored property of the build.
...
**Still not a clock**: a stalled build moves the meter by zero and pays zero, and a crew building at
a shallow floor halves its accrual and pays proportionally less gear for the same turns.

> #### THE QUANTUM IS SIZE-SENSITIVE NOW, and it cost nothing to make it so
>
> The meter's increment is in **work units** since `docs/plan_unit_costed_work.md`, so
> **a bigger job eats more gear, with no per-improvement authoring** — the thing this section used to
> record as impossible ("a build costs a fixed amount of gear whatever else is true of it"; a Steppe
> Runner's `Tame` and a rabbit's burned identical hurdles). A 75-unit Sow burns 1.5× what a 50-unit
> Cultivate does, and a Steppe Runner's 250-unit `Tame` burns 5× a rabbit's, because the species'
> `taming_cost_multiplier` prices the *job* rather than slowing the crew.
>
> **The declared amount was divided by 50 to keep that free** — the hurdles' `build_progress`
> **8.0 → 0.16**, where 50 is the reference job (`plant:tended`'s `work_cost`). Gear life per
> reference build is therefore *exactly* unchanged: `100 / 0.16 = 625` work units ≈ **12.5 gardens'
> worth**, the same 12.5 builds the old dial bought.
>
> **THE CHARGE INCLUDES THE UNITS THE TOOL PRE-PAID**, and the arithmetic is what makes that exact:
> the meter jumps to the raw cost on the turn it completes, so the before/after delta over a
> fully-geared 50-unit build sums to the whole 50 (33 worked, 17 pre-paid) — `50 × 0.16 = 8.0`
...
>
> **`hoes` ARE THE FIRST ITEM TO HEADLINE THE BUILD QUANTUM**, so theirs is the first gauge that
> reads *"≈12 gardens' worth left"* — they wear on `build_progress` and on nothing else, which is
> what the conversion was written for. The **hurdles** beside them declare `biomass_collected`
> **first**, and `headline_wear` is `wear[0]`, so their gauge reads *"≈2500 biomass butchered"* and is
...
> reference job, so headlining the build there would quote animal handling gear in the other web's
> unit. (`docs/plan_unit_costed_work.md` §6.3 reads *"one readout has to
...
**A ring is raised by the KEEPERS on the tending assignment.** `ExtendPen` is command-driven rather
than assignment-driven, so it has no crew of its own; it used to be handed a named
`PEN_EXTEND_CREW` of **one**, on the reasoning that the crew factor was the identity whatever was
passed. That reasoning expired with the crew cap — the crew is the build's throughput now — so the
constant is **deleted** and the arm passes the assignment's own `workers`, or every ring would be a
one-hand job beside a pen raised by the whole crew.
...
hand's `1.0` is `creatures.json`'s `person` row; `hurdles` and `wayfinding` declare the
...
- **A tier is an AGE, and the vocabulary is shared across items.** Every item's opening tier is
  **`plain`** — the bone, hide and fibre gear a band starts with — and the ages stack beside it in
  order: **`flint`** today on the three items stone can make, `bronze` the day metal lands, so the
  ladder is gated once (*"bronze needs Smithing"*) rather than per item.
- **`tiers[0]` is the default** — what a spawn stocks, what every reference rate resolves through,
  and the one tier `validate` forbids a `requires_knowledge` on. **The ORDER is the model**: the list
  is a `Vec`, not a map.
- ⛔ **WHICH TIER A CRAFT MAKES IS THE RECIPE'S TO SAY** (`RecipeOutput::tier`,
  `crafting.md` → "A RECIPE NAMES THE TIER IT MAKES"), and `validate_against` makes the declaration
  **mandatory** on any item with more than one tier. `ItemDefinition::craftable_tier` — *the best
  tier the faction knows* — survives as the fallback for a single-tier item and as the mechanism a
  knowledge-gated tier will need; it is not what resolves `spears` any more.
- **`plain` WAS CALLED `flint` UNTIL ISSUE #736, and NOT ONE VALUE MOVED.** The old name was a
  naming lie — every one of those tiers is made of bone, hide or fibre — and it became unusable the
  moment a real knapped tier existed. Grep it in tests and fixtures, not only in config.
- **THE FLINT TIER IS HARDER-HITTING AND BRITTLE, and the brittleness is the point.** `spears` 26
  attack against `plain`'s 20, `clubs` 9 against 6, `hoes` 0.7 `build_work` against 0.5 — all at
  `starting_durability` **70** against 100, so a flint spear takes 175 strikes where a plain one
  takes 250. **This is the first use of *"durability and performance are ORTHOGONAL axes"***: a
  straight upgrade retires the bone path one rung up, and a brittle one keeps both alive permanently.
  The two **road tools** deliberately gained no second tier — only their recipe's working end changed
  from bone to stone.
- **Still no bronze row.** An unreachable tier is dead content
  `SubsistenceSection.equipmentConfigJson` publishes to the Workbench, and metal has no producer;
  `flint` ships because stone has one *and* the book carries a recipe that makes it. A
  **knowledge-gated** tier is still covered by a **fixture** only
  (`a_tier_switches_an_items_attack_without_touching_its_shared_effects`), the same treatment
  `materials.json`'s `varieties` get — nothing shipped declares `requires_knowledge`, because the
  stone path is open on turn one and *doing* it is what teaches Knapping.
...
their `unequipped` side); the sled's and the baskets' own `plain` tiers declare the equipped `40.0`
...
> | ~~`systems/labor.rs` — a rung-3 **Field**'s managed collection cap~~ | **RETIRED, and the row is corrected rather than deleted so the claim is not re-derived.** It read *"`managed_per_worker_yield` / `_fodder` / `_trade`, `field_harvest_biomass` → `equipped_reference(ForageCarry)`"*. The whole rung-3 managed harvest went with `docs/plan_standing_upkeep.md` §4.10 (`forage.rs` → "RETIRED: `managed_per_worker_yield`"); **a Field is worker-capped through the ordinary `forage_take` path**, band-resolved like every other rung, so there is no band-less cap left to thread |
...
**A pen reads the hunt haul's rate — because it reads the hunt haul's STAT.** This paragraph used to
say *"`pen_carry` still shares the hunt haul's rate… `EquipmentStat::shares_equipped_rate_with` links
`PenCarry → HuntCarry`"*. `PenCarry` was the mechanism's only user and both went with issue #543: a
pen is collected on `HuntCarry` itself, so there is no second stat to keep in step and
`equipped_reference` resolves each stat against its own declaration. See "Carry is carry" above.
...
## The `expedition` job — one kit, EVERY way a ranging party keeps itself going

`KitJob::Expedition` is the ninth job, and the shipped `ranging` kit is the only roster entry that
carries gear for more than one activity at a time: `spears`, `sled`, `baskets` **and** `wayfinding` —
the two ways it feeds itself, plus the reach it does its actual job with.

**That is not a hole in "one item, one job" above.** Every item still declares exactly one stat and
serves exactly one activity — the sled cannot reach `forage_carry` and the baskets cannot reach
`hunt_carry`. What differs is **who is holding them**. A resident band's hunters and gatherers are
different people on different labour rows, so one kit per row is the honest split. A *detached
provisioned party* is **one crew** that gathers off a stand on one turn and takes a passing animal on
the next, and the two things it can do to feed itself are not two crews to equip separately.

**Which parties are on this job.** The **provisioned** missions — `ExpeditionMission::Scout` and
`Trade`. They walk out with a larder, drain `provision_upkeep_per_worker` every turn, and are the only
parties in the game that must replace what they eat while out of contact with their band
(`expeditions.md` → "Gather first, then hunt"). **A raid is not**: `Hunt` and denial parties live off
their kills, pay no upkeep, and resolve their kit off the **quarry's** derived default
(`fauna::quarry_default_hunt_kit`), so they stay on `KitJob::Hunt`.

**It is not a "does everything" kit, because coverage arms each item independently.** A party of ten
carrying four sleds and six baskets hauls at the sled tier on four crews and by hand on six, and
gathers at the basket tier on six and by hand on four ("The partly-equipped party"). A partly
equipped ranging party is therefore a real, modelled and ordinary state rather than a boolean, and the
four items wear on four different quanta — `strike`, `biomass_hauled`, `biomass_gathered`,
`tile_revealed` — so each runs down only for the work it actually did.

### `wayfinding` IS the fourth item, and it had to earn the place

It was **left out** when this kit was created, and that was right at the time: an expedition's
observation radius was the flat `expedition_config.json` → `observe_sight_range` and read no kit at
all, so the item would have been a thing in the party's hands that does nothing —
`config-loading.md`'s "looks live but isn't" in the one place a player would most reasonably expect it
to work.

**Making the radius kit-aware is what earned it the slot.** `observe_sight_range` is now the
*equipped* tier of `EquipmentStat::ExpeditionSightRange` and ships `9`; the item declares the bare
`6.0`, which is what that lever used to be flatly. So the fourth item buys **three tiles of reach**
and pays for them in durability, and a bare party sees exactly what it always saw — **nothing
regresses; it is pure upside for carrying the gear.** The full rationale, the charge site and the
turn-clock guard are in `expeditions.md` → "A party's reach is its KIT's".

### Two sight stats, one item — and they must not be collapsed

`wayfinding` declares `scout_vantage_range` **and** `expedition_sight_range`, which is legal (the
at-most-one-*item*-per-two-sided-stat rule is per stat) and necessary. The next reader will want to
collapse them; here is why that cannot work.

`rate_tier` takes the **equipped** side from the caller's baseline and the **unequipped** side from
the item, and an item declares **one** unequipped side per stat. Reusing `ScoutVantageRange` for a
party would therefore drag a bare-handed *party* down to the vantage's bare `1.0`. That is wrong:
`observe_sight_range`'s own doc says it matches the band base sight, and **a band standing still sees
that far with no gear at all**. The party is the same people with the same eyes — what the gear buys
is reach *beyond* unaided sight, never the ability to see at all.

A posted vantage (one or two people on a hilltop, equipped range 2) and a whole ranging party are two
different observers with two different **bare** values. Two observers, two bare values, two stats.

### A ranging kit MAY carry a mass-bounded weapon — and the band-wide rejection must not grow

`validate_warrior_kits_have_no_quarry` rejects a mass-bounded `attack` in a **Warrior or Scout** kit
because those roles resolve at `Quarry::Any`: there is nothing on the other side of that fight with a
`body_mass` to test the bound against, so the bound would be written, validated and then ignored.

**`KitJob::Expedition` is not in that list and must not be added.** A ranging party's roadside kill
resolves `PartyResolution::party_against(Quarry::Mass(..))` off the herd it actually met, exactly as a
hunt row does — so a snare in a ranging kit resolves *correctly* rather than counting everywhere. The
call site carries the same note, because the natural next edit is to "fix" the omission.
...
the material bought. A pen's rate is not a third carry number at all — it **is** `hunt_carry`, read
off the same tier by the same resolver ("Carry is carry").
...
2. **Wear is charged for USE, never for turns elapsed** (`docs/plan_denial_raid.md` §1.2 depends on
   ...
   JOB has its own quantum** (table above), and the **kit mask** is what keeps two jobs' gear apart
   where they share one (`strike`). Pinned by `wear_is_charged_for_work_not_for_turns_elapsed` —
   same world, same turn count, a scouting band loses exactly zero — by
   `the_sled_and_the_baskets_wear_on_different_quanta`, and by
   `each_kit_wears_on_a_use_quantum_of_its_own_job`, which asserts no hunt kit can reach the clubs.
...
**A SPAWN STOCKS A PARTY'S WORTH — `ceil(workers × start_stock_fraction / workers_per_unit)`, never
below one unit.** A unit arms `workers_per_unit` people (see "The partly-equipped party" below), so
the `count: 1` this used to stock would arm exactly **one** hunter of the shipped ~17-worker band and
send sixteen out bare-handed. **The half-again is the opening reserve**: coverage counts units in
*usable condition*, so at a flat `1.0` the turn a spear retires the party goes out one hunter short —
the reserve is what buys the band the turns between its first break and its bench.

**One unit is still one item's `starting_durability`** — the life the game has always had, and
untouched by this: a batch of `count` holds `count × starting_durability`, so what a party's worth
buys is *people armed*, not a longer-lived spear. `BandEquipment::start_stocked` — the **fresh
reference ledger** every quarry-scoring and roster-quoting surface prices against — still stocks one
unit each and takes no head count, because those surfaces read liveness and a count there would be
noise. The three seams, and which question each answers:

| seam | stocks | who calls it |
|---|---|---|
| `start_stocked(config)` | one unit each, ungraded | the reference ledger: `kit_supplying`, `quarry_default_hunt_kit`, the published kit roster, a launch forecast |
| `start_stocked_for(config, workers)` | a party's worth, ungraded | **the absent-COMPONENT fallback** — `advance_labor_allocation`, `advance_expeditions`, `snapshot/capture.rs`; "no ledger" means *outfitted*, and one unit no longer says that |
| `start_stocked_owned(config, recipes, materials, workers)` | a party's worth, stamped with the anchor grade | a **spawn**: `spawn_profile_population` and both expedition-outfitting paths |

Pinned by `the_reference_ledger_is_one_unit_and_the_shipped_lives_are_unchanged` (the lives, against
the literal use counts this file records) and `a_spawned_band_arms_every_worker_and_keeps_a_reserve`
(coverage is uniform at the band's own head count, with strictly more units than people behind it).
...
## The partly-equipped party — ten spears arm ten hunters, and the eleventh goes bare

**A party is no longer priced off the serving batch alone.** `EquipmentConfig::coverage(kit, workers,
wear)` is the one seam that divides a party's people by the gear it holds, and every unevenly-equipped
consumer reads it — the hunt, the raid, the haul, the assign-time seed and the in-flight ETA.

- **`ItemDefinition::workers_per_unit`** (serde default `1`, `validate` rejects `0`) is the item's own
  statement of how many people one unit takes. Every shipped item is `1`; the axis exists because a
  four-worker net is the first thing that is not, and **a unit needs its FULL crew** — ten workers and
  three four-worker nets crew two nets with eight people and leave two unequipped, never a third of a
  net. A **one-worker** item is exact rather than floored, deliberately: a forecast counts hunters in
  fractions (`hunt_engage_workers`), and flooring the common case would put the projection a whole
  person away from the take it is projecting.
- **The partition is by ITEM SET, and the shortfall is PER ITEM.** Each item covers a prefix of the
  party, so sorting the coverages and cutting at each distinct boundary yields runs holding identical
  gear: ten hunters with five spears and ten sleds is **two crews**, five on `{spears, sled}` and five
  on `{sled}`. A `Crew` carries the party's own `KitChoice` **narrowed to what that run holds** — the
  same kit with fewer things in it — which is what lets `hunter_profile_against`,
  `hunt_per_worker_biomass_capacity`, `exposure`, `dispersion` and `multiplier` all answer for a crew
  with **no new arm**.
- **A dead unit covers nobody** (`BandEquipment::live_units`), so the durability cliff now arrives one
  person at a time. That graded disarmament falls out of the counts rather than being laid on top.
- **`KitCoverage` never hands back an empty list for a party that exists** — a wholly bare party is
  one crew holding nothing — so no consumer needs a "no crews" branch. A party of *nobody* has no
  crews, and `KitCoverage::weighted_rate` answers such a coverage at the chosen kit's own rate, which
...
- **An item nothing asks for is not rationed.** `demand == 0` means no budgeted row carries it, so
  ...
  **party** (empty allocation) reading exactly what the ledger-wide `coverage` gave it. ⛔ It used to
  name the **builders' pool** as the second example, *"whose kit comes from the build queue rather
  than from a row"* — an accident of `kit_choice` answering `none` for that row, not a property of
  the pool, and the last place one band's gear was issued twice. See "A STANDING POOL'S KIT IS NOT
  STORED ON ITS ROW".
...
**ONE COVERAGE IS OUTSIDE IT — the scout vantage** in `visibility_systems`, which runs at its own
point in the turn against a pool of its own. **The overlap there is vacuous rather than absent**,
which is a different claim and the only one this file should make: no shipped kit outside the
expedition roster carries `wayfinding`, so the Scout row's demand is its own and its share is the
whole stock either way. That is a property of today's roster, not of the seam — a second
scout-capable kit would end it.

> #### ⛔ THE BUILDERS' POOL AND `keeping_rates` WERE OUTSIDE IT TOO, AND THE BOUNDARY IS RETIRED
>
> This paragraph used to read **"Three coverages are deliberately outside it, and this is a boundary
> rather than an oversight"**, naming the **builders'** pool (`BuildersGear::for_source`) and
> **`keeping_rates`** beside the scout vantage, on the grounds that *"folding them into one band-wide
> allocation is a change to **when** each pool is resolved, not a spelling correction."* A first
> correction under it retired the claim that the boundary cost nothing — *"Nothing shipped puts one
> item in a take kit and one of those, so no stock is over-issued today"* — and left the over-issue
> standing as a named consequence.
>
> **The boundary is gone now, both ends together.** Four shipped kits serve `builders` **and** a role
> job — `tillage` (hoes) `["builders", "agriculture"]`, `hurdling` (crook)
> `["builders", "husbandry"]`, `roadbuilding` and `paving` `["builders", "roadwork"]` — so the pools
> and the budgeted rows really did reach for one stock, and six hoes armed six builders **and** six
> keepers. `LaborAllocation::row_kit` registers each standing pool's demand and the pool cuts its
> take from that row's share; the "when" objection never bit, because every one of those pools is
> resolved inside `advance_labor_allocation` (`settle_bands_roadwork` and `settle_bands_extraction`
> are called from it, not scheduled beside it).
>
> ⛔ **A CORRECTION UNDER THAT CORRECTION, because it was load-bearing and false**: the retirement
> note claimed each of those items was reachable *"from a role row that names it through
> `kits_for_job` — which the role kit picker and the `assign_labor <role> <n> kit <id>` grammar both
> expose."* **No role row can name one.** The four jobs those kits serve are exactly the four
> standing pools, and `assign_labor` refuses a `kit` token on every one of them
> (`staffing_a_standing_pool`), so their stored kit is always `None` and `kit_choice` always answers
> `default_kits.<job>`, which is `none` for all four. The over-issue was therefore **pool against
...
| row | resolves | why |
|---|---|---|
| `builders` | `LaborAllocation::builders_kit` — the **head entry's** kit | **all hands go on the head** (`build_workers` is `builders` only for the head and `0` for everything below it), so at any instant the pool funds **one** queue entry. Its demand is `builders` workers on that entry's kit, registered **once** |
| `agriculture` / `husbandry` | `keeping_kit_for(None, branch, None)` — `tillage` / `hurdling` | the *selection* is still per work site; what the row states is the derivation every site departs from |
| `roadwork` / `quarrywork` | `kit_choice`, i.e. `none` | **a property of the roster, not of the seam**: every tool serving those webs declares a `rung` (`earthmoving` on `route:dirt_road`, `stone_dressing` on `route:paved_road` / `extraction:quarry`), a role row stands on no rung, and `EquipmentEffect::serves_build` refuses a rung-bound tool where none was named. Their pools are still **cut** from the budget; what they cannot do is register against it |

> ⛔ **ONE READING PER BRANCH WOULD DOUBLE-COUNT THE SAME PEOPLE.** `BuildersGear::for_source` is
> called once per source the band works — the Forage arm, the Herd arm, the deposit arm and the road
> arm each ask it — and on the two food webs those resolve *different* kits. Registering `builders`
> once per branch would put the whole pool's head count behind two tools at once, which is the error
> this arc exists to remove, reintroduced on the demand side. The head is what makes "registered
> once" well-defined: everything below it banks nothing and is only **dated**.

**The take is cut from that same row's share**, and the two ends had to move together or neither
should have: registering the demand without cutting the take starves every row beside the pool while
builders still arm in full; cutting the take without registering the demand under-arms the pool
against a denominator that never counted it. `for_source` builds
`BandItemBudget::with_prospective_row(rows_excluding_source(Builders), kit, builders)` — the *"a
prospective row competes exactly as a committed one does"* seam, one level over. On the **head**,
whose kit is the one the row registered, that is arithmetically the committed budget, so **the wire
and the take state one number**; an entry further down the queue, which banks nothing, is dated at
what it *would* be armed with when the pool reaches it.

**`keeping_rates` cuts the same way, per kit group.** Grouping by kit id keeps one pool's sites from
double-counting each other and says nothing about the pools beside it, so each group's coverage is
now struck against the band's budget with the group's own `(kit, hands)` chained onto the band's
other rows. With one group and no per-site override that is exactly the committed budget, which is
what keeps an unshort band bit-identical.

> **What the capture lost is an arm, not a rule.** `snapshot::population`'s `row_gear` carried a
> `match` whose `Builders` arm kitted the row from the queue and covered it off the **whole** ledger,
> precisely because the take did. Kit and share are one resolution now, so every row goes through one
> expression — and the defect that `match` was written to prevent (the row kitted one way and
> budgeted another) is closed at the source instead of guarded at the readout.

**What a short band now publishes**: the pool's claim *is* in the budget, so a role row saturating
the stock beside a staffed pool no longer puts more hands on a tool than the band owns — six hoes
read `workersHolding 6`, split pro-rata by head count, where they read `12` before.
`build_queue.rs` pins it in three:
`a_builders_pool_and_a_keeping_row_cannot_arm_more_hands_than_the_band_owns` asserts on the **take**
from both ends (`buildWorkFromGear` inverted to hands, and `upkeepSupplied` inverted through the
keeping rate, with the keepers held short of their bill so that number is the rate and not the capped
demand);
`a_builders_row_and_a_keeping_row_sharing_one_tool_publish_the_reach_the_turn_arms` keeps wire equals
take and asserts the shared number now **falls** when a keeping row competes — the assertion that
moved, and the indifference it used to assert *was* the double-issue; and
`a_band_that_is_not_short_arms_both_pools_in_full` is the abundant control, where each pool takes
exactly what it would take standing alone.

**Still outside all of it: a DETACHED EXPEDITION**, unchanged — a launched party carries its own wear
ledger and works no source rows, so it is the empty-allocation case the *"an item nothing asks for is
not rationed"* rule already answers.

### `HuntingParty` carries its composition, in SHARES

`HuntingParty` is `{ crews: Vec<HuntCrew>, tuning, dispersion }` and is **Clone, not Copy**;
`HuntCrew` is `{ share, hunter, injury_damage_per_animal }`.

> **`share` is a FRACTION of the party, never a head count** — that is what keeps the struct
> **scale-free**, exactly as the single-profile form was. Every call site still passes its own
> `hunters` count, and a sub-party that engages (`hunt_engage_workers` returning fewer than were
> assigned, or a build dip shrinking the crew) carries the *same mix* rather than an arbitrary prefix
> of it. Baking a head count in would make the party disagree with every caller that scales it.

- **`HuntingParty::uniform(...)`** is the shape a fully-covered party, a fixture, or a
  reference quote resolves to — one crew at `share 1.0`.
- **`PartyResolution::party_against(quarry)`** is the live seam: equipment ⊕ coverage ⊕ ledger ⊕ the
  intrinsic `person` row ⊕ the resolver dials, with the quarry held back as a **late** argument
  because a mass-bounded weapon cannot be resolved before the target is known. That is the
  `party_for = |body_mass| …` factory `advance_labor_allocation` and `advance_expeditions` already
  used, given a name so the coverage behind it is resolved once per party per turn.
- **`dispersion` is PARTY-WIDE and is the MAX across the crews.** The same clause
  `KitChoice::multiplier` resolves an item's multipliers with, one level up: if part of your party is
  running up and throwing spears, the herd is scared — the trap line the other half set does not
  un-scare it. So buying the quiet approach means buying **enough** of it. `stayers` / `stay_fraction`
  are unchanged.
- **`injury_damage_per_animal` is per CREW**, because `exposure` is a kit's: the half holding the
  stand-off device takes the stand-off's hazard. `hunt_injuries` gives each crew `stayed × share` of
  the engagement and caps it at that crew's own people, so a **uniformly** exposed party is
  arithmetically identical to the single-profile form it replaced.

### The two adapters build one contingent per crew

**`fauna::resolve_hunt_fight`** emits one `Contingent` per crew (`count = hunters × share`, profile =
that crew's hunter, id `person#<index>`), and **the resolver is untouched**: `combat::resolve_fight`
already gates every attacker/target pair on the attacker's own `attack`, so a bare-handed run inside a
speared party lands *exactly zero* on a defence it cannot clear. One averaged profile would have let
...
- **The mean is CREW-WEIGHTED, and that is what makes the cap correct.** Five sledded hunters and
  ...
  real total and the inversions (`hunt_haul_workers`, the gather's `workers_needed`) invert that same
  product. A per-crew carry would need both to grow a crew loop for an answer identical to this one.
- **The plant web is not an exception, and it was one for exactly one slice.** *"Gear covers people,
  not jobs"* is the whole model: a band with five baskets and sixteen gatherers gathers with five
  baskets. Pinned by
  `a_band_short_of_baskets_gathers_between_the_bare_and_the_basketed` (strictly between bare hands
  and a full set, on real turns) and `a_crew_short_of_sleds_carries_between_the_bare_and_the_geared`
  (the same shape on the haul, plus the weighted-mean identity).
- **The REFERENCE surfaces stay uniform** — a herd row's quote, the published kit roster, a launch
  forecast, and `snapshot/population.rs`'s per-cohort tiers. Each describes *what a kit buys*, priced
  against the one-unit reference ledger; how much of it a band owns is a different question, and the
  band's own take answers it.
...
> change.** `resolve_fight` sizes a side's losses from the *enemy's* power over its own **head
> count**; a defender's `attack` reaches only the enemy's losses (which the raid discards) and
> `force_power`. So what a partly-clubbed line changes today is what the pack takes, not what the camp
> loses. `a_band_short_of_clubs_stands_up_an_armed_line_and_a_bare_one` pins the composition and says
> so.

**`has_condition` keeps its meaning for every existing caller** (*the band owns at least one unit with
condition*) — coverage asks the *count* question through `live_units` beside it, so the liveness
predicate did not have to change.
...
**`wear_kit` names the QUANTUM, not the items.** A wear site says *"this party just landed N blows"*
and every item in its kit that wears per blow is charged — so **an item added to a kit is charged with
no call-site edit**, and an item the kit does not carry is never charged at all. That last clause is what
...
- ~~**NO PLANT ITEM DECLARES `build_rate` yet.**~~ **`hoes` ship**, in a `tillage` kit of their own,
  and the question that bullet left open — *which kit pays for one* — was answered by giving each web
  its own **builders** kit rather than by loading a digging tool onto the gathering kit. What a
  builders kit gives up is the whole of the rest of the roster: the pool that carries it carries
  nothing else, and a band that wants both webs' tools has to hold both.
- ~~**A Field's (rung-3) managed collection cap stays on the equipped reference rate.**~~
  **CORRECTED — every clause of it was false, and it is quoted rather than deleted because this is the
  paragraph issue #543 was filed on.** It read: *"Rung 3's harvest is quoted per account
  (`managed_per_worker_yield` / `_fodder` / `_trade`) and draws no biomass down, so it has no single
  biomass quantum to charge a basket against."* The whole rung-3 managed harvest was retired with
  `docs/plan_standing_upkeep.md` §4.10 — **a Field is drawn down through the ordinary `forage_take`
  path, band-resolved at the ordinary per-gatherer throughput, exactly as rungs 1–2 are.** There is no
  Field arm, no managed rate, and no band-less cap. **Every rung on the plant web is
  basket-resolved.**
...
- **A detached party resolves and wears its own kit.** `advance_expeditions` queries
  ...
  `hunt_per_worker_biomass_capacity`, and charges the weapon **per blow its crews landed** + the sled
  per biomass hauled — the same use quanta a resident band pays. Before slice 4 a raid ran on free,
...
## Wear follows the work actually done — the `strike` quantum

**A weapon is charged for the blows it landed in a body, and for nothing else.** `WearQuantum::Kill`
and `WearQuantum::Fight` are **both retired into `WearQuantum::Strike`**, because both charged gear
that did no work:

- **`kill` billed the whole party for a body one part of it brought down.** Ray's case: *ten hunters
  bring down five deer, two hunters per deer, but only two deer are available — only four hunters'
  spears should be charged.*
- **`fight` billed the whole warrior line for an engagement most of it may never have swung in.** A
  band with three clubs and eight warriors paid one use of its clubs whatever happened.

### The rule

```text
charged strikes = strikes_landed × (damage_absorbed / damage_dealt)
```

- **`strikes_landed` is the STRIKER'S**, not the target's — `combat::ContingentResult::strikes_landed`
  is *"how many blows this contingent landed on the enemy"*, deliberately facing the other way from
  the `killed` / `wounded` / `damage_dealt` beside it, because wear happens on the swing. The
  resolver already drew the number (`landed_strikes`); it simply used to discard it.
- **A gated pairing lands NOTHING.** `combat::resolve_fight` skips an attacker whose `attack` does not
  clear the target's `defense` before a strike is drawn, so a bare-handed run inside a speared party
  is charged a hard zero — and holds no weapon to charge anyway. **The two halves are separate
  guarantees and are asserted separately**
  (`a_bare_handed_crew_is_charged_no_strikes`).
- **`damage_absorbed` is `min(dealt, standing × durability)`** — `combat::damage_absorbed`, the one
  home of the clamp `DamageLedger::strike` was already applying and the raid needs without a ledger.
  The hunt reads it back off the ledger (`DamageLedger::strike_blow`) rather than recomputing it, so
  `standing` has one authority.
- **Charged PER CREW, through each crew's own narrowed kit** — `fauna::StrikeCharge` rides out on
  `HuntFight::strike_charges` and every take path calls the one seam
  `HuntFight::charge_strike_wear`, so a wear site says *"this party just fought"* and each crew pays
  for its own swing. A party built through `HuntingParty::uniform` carries no kit and is charged
  nothing: a fixture or a launch quote has no ledger behind it, which is a different statement from
  *"it swung and wore nothing out"*.
- Ray's arithmetic is the test, not an illustration:
  `the_strike_charge_is_the_share_of_the_blow_the_bodies_could_absorb` runs ten hunters at ten damage
  a blow against two standing twenty-durability bodies — `2/5` absorbed, `10 × 0.4 = 4` charged — on
  **both** the one-sided fast path and the full resolver, because a charge that differed between them
  would be a second wear model.

> **Where the party OVERKILLS, the charge collapses to a clean identity**: with `absorbed = kills ×
> durability` and `dealt = strikes × per_strike`, the charge is exactly
> `kills × (durability / per_strike)` — **the blows it took to put those bodies down**. Where the
> party *under*-kills (everything it dealt went into a live animal) the ratio is `1` and every strike
> is charged, which is the honest reading of a party that swung at animals still standing.
...
floor, the roster's authored `wariness 0.65`, its own spawn stock of 26 spears:

| | per turn | per spear UNIT |
|---|---|---|
| kills (the retired `kill` quantum) | ~6.0 | ~42 turns |
| **charged blows (`strike`)** | **~7.9** | **~31.7 turns** |

**A ~1.3× faster burn, and the factor is not a coincidence**: it is `durability / per-strike damage`
= `25 / (20 − 1)` on this quarry, the identity above. It is *not* the naive `strikes = hunters` (17 a
turn, ~15 turns) — the absorbed clamp is what keeps overkill against a wary, thinned herd from being
billed as work. A quarry with a lower `durability : damage` ratio burns gear proportionally slower;
one that soaks many blows per body burns it faster, which is the retune's whole shape.
...
> A corralled herd's default is the kit that supplies `EquipmentStat::HuntCarry`; every other herd's
...
pen is nil. A pen has no fight for the scorer to price, so the one axis it can still be scored on is
the **carry**, and the earliest hunt kit supplying `HuntCarry` is `big_game` (see "Carry is carry" —
this picks a kit to OPEN ON and never a rate). That is the same axis the picker's greying and
...
> **⛔ It is NOT that a pen resolves no fight.** It does, at the species' own `defense`, since §4.9
> item 12b (`husbandry.md` → "THE TAKE RUNS ITS THREE STAGES AT EVERY RUNG"). What the *scorer* cannot
> price is the stalking approach a pen never makes — `per_hunter_take_biomass` reaches against
> `engage_rate` at the wild retreat, and there is neither at a pen. The source-axis rule stands on the
> **carry** being the one axis left to compare kits on, not on an exemption from fighting.
...
| `kitTiers:[BandKitTiers]` | **What EVERY offered kit would grant this band, at its live wear** — one row per roster kit (`kitId` + **eight** of the tiers `KitOption` carries: `attack`, the two mass bounds, `huntCarryPerWorkerBiomass`, `forageCarryPerWorkerBiomass`, `scoutVantageRange`, `dispersion`, `exposure`), **plus the build trio and the carry coverage pair** — the two things that are facts about the band's LEDGER rather than about the kit, so `KitOption` cannot carry them. See below: it is the resolved answer, and a client must not re-derive it. **`expeditionSightRange` is the one `KitOption` tier that has no twin here**: it describes what a *detached party* observes at, resolved against that party's own kit and its own wear, so a resident band's live-ledger row has no reading of it to give |
...
**The last two were published per KIT before they were published per BAND**, and that gap is what
they close: `KitOption` has carried `scoutVantageRange` / `attack` since the roster expanded, so the
picker could quote a fresh kit's numbers while no readout could state a scout's actual reach, a
...
| Field | Meaning |
|---|---|
| `SubsistenceSection.kits:[KitOption]` | **The roster, once per world** — `id`, `displayName`, `jobs`, `itemIds`, and the tiers each kit grants a party whose components are **fresh** (`attack`, `huntCarryPerWorkerBiomass`, `forageCarryPerWorkerBiomass`, plus the appended `scoutVantageRange` and `expeditionSightRange`), so the picker renders real numbers without a second copy of the TOE table. **The two sight fields are not one number read twice** — a posted vantage and a ranging party are different observers with different *bare* readings (1 tile against 6), so a launch sheet quoting the vantage's would understate a bare party's reach sixfold |
...
|---|---|---|
| `hunterAttack`, `huntCarryPerWorkerBiomass` | the **hunt** default | `PopulationCohortState.kitId` |
...
**A PEN IS ON THE HUNT ROW and needs no field of its own** — a pen is worked from a Hunt assignment
*and on the Hunt row's own carry* (issue #543), so `huntCarryPerWorkerBiomass` answers for it and
`kitId` answers for that.
...
**These five answer at the JOB DEFAULT, and that is a different question from the one `kitTiers`
...
kit and the picker offers all of them. `scoutVantageRange` rides **both tables** for exactly that
reason, and the flat field is not redundant with the rows beside it.
...
out with the **stalking** kit, whose appended tiers all differ from the job default each would
otherwise resolve to, so a resolution reaching for a default fails every one.
