...
- ~~The Crafter role and replenishment/upgrade are out of scope.~~ ~~What remains of #494 is the
  **count** and the quality **tier**.~~ **All three are wired**: replenishment (`crafting.md` → "The
  ...
  remains of the arc is the **wire and the panel** (§7): nothing publishes a batch's count, tier or
  grade to a client yet, so the *"turns left"* readout the counts make real has no field to ride on.
