...
## The `expedition` job — one kit, BOTH ways a ranging party feeds itself

`KitJob::Expedition` is the ninth job, and the shipped `ranging` kit is the only roster entry that
carries gear for two activities at once: `spears`, `sled` **and** `baskets`.
...
three items wear on three different quanta — `strike`, `biomass_hauled`, `biomass_gathered` — so each
runs down only for the work it actually did.

**`wayfinding` is deliberately NOT in it.** An expedition's observation radius is the flat
`expedition_config.json` → `observe_sight_range` and is **not kit-aware**; the wayfinding item raises
`scout_vantage_range`, which is a *resident* band's posted vantage. Putting it in the ranging kit
would hand the party an item that does nothing — `config-loading.md`'s "looks live but isn't" in the
one place a player would most reasonably expect it to work.
...
| Field | Meaning |
|---|---|
| `SubsistenceSection.kits:[KitOption]` | **The roster, once per world** — `id`, `displayName`, `jobs`, `itemIds`, and the tiers each kit grants a party whose components are **fresh** (`attack`, `huntCarryPerWorkerBiomass`, `forageCarryPerWorkerBiomass`, plus the appended `scoutVantageRange`), so the picker renders real numbers without a second copy of the TOE table |
