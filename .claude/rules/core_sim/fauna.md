...
> `market` block (and `labor_config.json`'s `forage.market`) **keep their old key names** — the fauna
> one is retired later in the same arc, the plant one in a later plant-side pass.
>
...
> as Deplete's floor). Config: the `hunt.{surplus_multiplier, deplete_multiplier}` + `market` blocks of
> `fauna_config.json`.
