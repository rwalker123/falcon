...
> >
> > **⛔ AND THE PEN CURVE IS LITERALLY THE SAME CURVE NOW.** The pen's own
> > `pen_crew_take_curve` published `quantise_animal_take(…).killed as f32` — whole animals — while
> > every stalking row beside it published the un-floored rate, so a **penned** aurochs whose
> > next-turn room is 54 biomass read `0.0` at every crew size, `hunt_useful_crew` fell to
> > `NO_USEFUL_CREW`, and the Work board's `+` shut on a pen collecting a beast every two and a half
> > turns — the same cadence-as-a-never, one function over. It was fixed to publish a rate, and §4.9
> > item 12b then retired the function outright: a pen runs the same three stages the range does, so
> > `hunt_crew_take_curve` is one body with a corralled row taking `.min(keepers' carry)`
> > (`husbandry.md` → "A PEN'S CREW CURVE IS THE STALKING CURVE PLUS ONE `.min()`").
> >
...
> covered for it. **Since §4.9 item 12b the pen has an engagement stage like every other rung** and
> spends the room in `resolve_hunt_engagement` with the range (`husbandry.md` → "THE TAKE RUNS ITS
> THREE STAGES AT EVERY RUNG"), so a caller that forgets is a **stripped herd in a test**, not a
> silent save.
>
...
>   Sustain-hunted slow herd sitting near an `animals_per_herder` multiple (a Wild Aurochs near 40
...
>   | term | unit | rate |
>   |---|---|---|
>   | `herd_herders_needed` | **heads** minded | `animals_per_herder` (one herder minds 40 aurochs) |
...
>     times `husbandry.pen_engage_gain`) and its `stay` is its own rung-scaled retreat
>     (`herd_wariness`, `husbandry.pen_wariness`), so this term sizes the crew that walks the room
>     out. **A penned animal IS NO LONGER FOUGHT** (PR #618) — `fauna::herd_fight_stage` returns
>     `None` at `animal:pen` and everything that stays is slaughtered. `SourceYieldForecast::fight`
>     nevertheless stays `Some(..)` at every rung, and that is deliberate rather than leftover: that
>     tuple is what the **retreat** reads (`pen_wariness`), so nulling it would delete the retreat
>     along with the fight. The slaughter is carried by its own field, `SourceYieldForecast::
>     slaughters`. `NO_RETREAT_STAGE_STAY` survives for the plant web alone (`husbandry.md` → "THE
>     TAKE RUNS ITS THREE STAGES AT EVERY RUNG").
...
- *Yield*: **none here — passive-free pastoral is RETIRED** (intensification ladder slice 3b, §3:
  ...
  (`r` = wild × `pastoral_gain` 1.25), so the *same* hunters take more sustainable food from the
