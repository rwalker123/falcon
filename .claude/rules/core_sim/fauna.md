...
>     out. **A penned animal IS fought** since §4.9 item 12b — `SourceYieldForecast::fight` is
>     `Some(..)` for every animal source at every rung, and `NO_RETREAT_STAGE_STAY` survives for the
>     plant web alone (`husbandry.md` → "THE TAKE RUNS ITS THREE STAGES AT EVERY RUNG").
