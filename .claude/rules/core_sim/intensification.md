...
- **THE GEAR LANDS ON THE JOB, NOT ON THE CREW** (`docs/plan_unit_costed_work.md` §6). A tool takes
  ...
  *percentage* of turns on a garden and on a farm alike — the shape the arc exists to escape.
  ...
  `equipment.md` → "The build axis"; the clamp is `LadderConfig::min_build_fraction`.
...
- **`effective_build_cost` — what the CREW BROUGHT** (issue #515, `equipment.md` → "The build
  ...
  `max(cost × min_build_fraction, cost − t)`. `intensification::NO_BUILD_GEAR` (**`0.0`**) for a crew
  carrying nothing that helps — every plant build today, and every animal one whose crew left the
  handling gear at camp. It touches **neither** `build_accrual` **nor** `build_decay` **nor**
...
- **`min_build_fraction` — the floor a tool cannot shrink a job past**, a top-level ladder lever
  (`0.25`, validated `0 < f <= 1`). It exists *because* gear is additive off the cost now: nothing
  else stops a rich enough kit driving a cheap rung to zero, and a rung that completes the turn it is
  picked is not an investment. It lives on the **ladder** because the ladder owns build costs — the
  gear declares what it takes off, and how far that may go is a property of the jobs.
