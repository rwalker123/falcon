...
- **`build_accrual`** returns `progress_per_turn × timescale × crew_scale(workers)` **only** when
  `policy` **is** the rung's own `verb` *and* the caller's rung-specific gates hold (`eligible` — knows
  the unlock knowledge, source healthy, species ceiling allows, faction owns it); otherwise `0`. **A
  rung with `verb: null` is never driven** — which is what keeps the two `wild` rungs (nothing to
  build) out of the engine.
