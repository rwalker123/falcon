...
**THREE rung gains are overridable today** — `pastoral_engage_gain`, `pastoral_resistance` and
`pen_engage_gain`. The third was added when the pen retune showed the same problem one rung up: a
flat handling rate let one keeper work nearly a whole aurochs herd while a fowl coop needed five, so
the crew a pen wants was a property of the species and not of the dial. The remaining gains stay
global because nothing has needed them per-species; adding one is two fields and a resolver on the
same pattern.
