...
### An out-of-range source POSTS A WORK PARTY, and is never parked at `+0.00`

**⛔ This section used to say the opposite, and the reversal is the point.** A Forage row past
`band_work_range` and a Hunt row past `hunt_reach` were **abandoned** — the workers returned to the
pool and a feed entry named the tile — because a kept assignment reads a *correct* `+0.00` forever
while the tile still renders as worked and its labour is silently dead. `+0.00` must mean *"this
source is barren"*, never *"your band is elsewhere"*.

The work party closes that the other way round (`docs/plan_civilization_steps.md` §One work party,
`.claude/rules/core_sim/work-party.md`): the row acquires a **party** instead of lapsing, pays a
real reduced rate from where it stands, and publishes its travel, its porters and the steady rate
arriving home. *"The workers are still the band's, they are just somewhere else."* So a worked row
is never a dead one, which is the whole of what the lapse was protecting.

**A source inside `band_work_range` takes no party at all and is priced exactly as it always was** —
the identity the model rests on. The threshold is `band_work_range` for **every** job: a Hunt row gets
no longer apron than a Forage one, so a hunt three to five tiles out now posts a party where the
...
**The assign-time seed still declines a far row.** `seed_source_yield` returns early past range, so
a fresh posting seeds `0` until its first resolved turn. That is honest rather than merely absent —
a party still walking out genuinely delivers nothing — but the steady figure does not appear until
the turn resolves.
