...
`.claude/rules/core_sim/work-party.md`): the row acquires a **party** instead of lapsing, works
the source from where it stands, walks its take home a pack at a time, and publishes its walk, the
hunters on the road and the rate arriving home. *"The workers are still the band's, they are just somewhere else."* So a worked row
