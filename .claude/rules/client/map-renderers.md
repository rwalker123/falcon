...
paths:
  ...
  - "clients/godot_thin_client/src/scripts/ui/BandSourceList.gd"
---
...
| `ui/BandMarkerRenderer.gd` | Owns MapView's PRIMARY player-band markers: the offset card-stack of settlement-stage tokens / expedition flag-discs, the faction nameplate banner (+ its reused StyleBoxFlat), the name PILL and its overlap cull, the food-days dot, the travel/task arrow, and the ×N over-cap count pill. `_view: MapView` back-ref; `draw_primary_bands()` called during MapView's `_draw`. `_name_pill_rects` measures a nameplate ONCE and three readers spend that measurement: the cull (`_label_rects`), the draw pass (`_label_grants`), and `name_pill_offset(tile)` — the same FOOTPRINT rect expressed relative to the token centre, which is what `BandSourceList` docks clear of (`map-markers.md`). Pixel-identical to the old inlined code (verified via `map_preview` byte-diff) |
...
| `ui/TerrainRenderer.gd` | Owns MapView's TERRAIN raster/shader subsystem: the Approach-B per-pixel biome-blend shader (the whole-map `TerrainBlendQuad` child + its `ShaderMaterial` + the seven per-hex splatmap textures it is fed — id / vis / elev / river-edge / river-channel / navigable-underlying / road), the blend-OFF per-hex texture cache, the blend-class + canopy/peak code maps, and the `T`-key texture toggle (MapView keeps thin `get_terrain_textures_enabled` / `enable_terrain_textures` pass-throughs for the Inspector/HUD; `CachedMapRenderer` reads the cache via `hex_texture_for`). **All nine tuning-const families live here** (`EDGE_BLEND_*` / `WATER_BLEND_*` / `SHORE_*` / `CANOPY_*` / `PEAK_*` / `RIVER_*` / `ROAD_*` / `BASE_DEFAULT_TEXTURE_SCALE`); `EDGE_BLEND_MIN_RADIUS` + `FOW_EXPLORED/VISIBLE_THRESHOLD` are `const X = MapView.Y` aliases, so each has exactly one definition. `_view: MapView` back-ref — every draw command plus the shared geometry/colour/visibility/river primitives stay on MapView. Pixel-identical to the old inlined code, **verified by byte-diff**: an extracted run matched a pre-extraction run on all 286 `map_preview` + `blend_probe` frames (the 4 `map_band_*` frames that vary run-to-run do so identically before and after — the `map_preview` window-maximize race, not this change) |
| `ui/BandOverlayRenderer.gd` | Owns MapView's SELECTED-BAND / SELECTED-HERD overlay family: the two range borders (`FORAGE`/`SCOUT_RANGE_OUTLINE*` — the red `HUNT_RANGE_OUTLINE*` ring at `hunt_reach` is retired, a caravan's only threshold being `band_work_range`) + worked-forage fills + hunted-herd rings/links, the dashed-amber optimistic PENDING overlay (`LABOR_PENDING_*`), the travel-destination line + reticle (`TRAVEL_DEST_*`), the selected herd's graze-range ring (`HERD_RANGE_*`), the corralled herd's pen footprint (`PEN_FOOTPRINT_*`), the deferred per-source BADGE batch (the `⚒N` plate, its rung glyph and the build ARC on the ring), and **THE SELECTED BAND'S SOURCE ROWS** — `compute_source_rows`, the model behind the docked `BandSourceList` (see its row below) and the anchors that panel's leader lines run to. **Every one of those const families moved here** — each was measured to have no executable reference outside the family, so none needed a `const X = MapView.Y` alias (unlike `TerrainRenderer`); `ICON_MIN_DETAIL_RADIUS` is read as `_view.ICON_MIN_DETAIL_RADIUS`, the `SecondaryMarkerRenderer` idiom. `_view: MapView` back-ref — every draw command plus the shared geometry/hex/glyph/pill primitives (`_hex_center` / `_hex_points` / `_fill_hex` / `_outline_hex` / `_hex_distance` / `_band_effective_col` / `_wrapped_col_delta` / `_is_tile_visible` / `_herd_by_id` / `_draw_marker_glyph` / `_draw_reticle`) and the unit/herd/selection state stay on MapView. It owns only this family's own state: the pushed `_labor_pending` map, the per-frame `_deferred_source_badges` batch, and the per-frame row model (`_source_rows` / `_selected_band_center` / `_selected_band_tile` / `_source_total_text`). **FOUR entry points, and their ORDER in `_draw` is the contract**: `draw_band_work_highlights` / `draw_herd_range_highlights` / `draw_pen_footprint_highlight` first (over the tile tints, under the markers), then **`flush_yield_labels` LAST** — after markers, rings, links, pending overlays and targeting, because those layers used to paint over the plates. **THE BADGE LIFECYCLE IS WHOLLY INSIDE THIS FILE**: `draw_worked_source_marks` CLEARS the batch and queues one entry per worked source — the one place a source's BUILD state is resolved (`SourceForecast.build_is_stalled`) — and `flush_yield_labels` renders it WITHOUT draining, so this frame's entries stay readable after the frame (`badge_rung` is how `map_preview` asks a plate what it says). **THE NAME OUTLIVED THE LABELS ON PURPOSE**: the per-source RATE PILLS this batch was built for went to `BandSourceList` in issue #650 and took `_deferred_yield_labels`, `_draw_yield_label`, `_queue_yield_label` and the `show_yields` far-zoom LOD gate with them — a screen-space panel needs no zoom gate, so ROWS are built at every zoom — but `MapView` reaches this seam by name, so the name stays. ⛔ **Do not re-add a pill collision / lift / stagger pass**; nothing here places anything relative to anything else (`overlay-channels.md` has both cures and why the second one failed). **MapView's own selection/hover outline is drawn AFTER all three of those passes** (and still before the markers, so a token reads on top): each stamps a per-tile `_outline_hex` across its ENTIRE disk — the worked-forage one at the selection line's own 3.0 width — so an outline drawn ahead of them is erased on every tile inside a graze / prey-sense / pen disk (issue #405). The selection border is the topmost tile border; punching a hole in a disk instead would misstate the sim's range. `map_preview`'s `map_pasture_herd_range` / `map_predator_prey_sense` / `map_pasture_pen_footprint` states each select an OFF-ANCHOR tile inside the disk, which is what puts that collision in a frame. **`set_labor_pending` stays PUBLIC ON MAPVIEW as a thin same-named pass-through** (it stores here, MapView owns the `queue_redraw`) — `Main.gd` wires the HUD's `labor_pending_changed` signal to it BY NAME via `has_method`/`Callable`, and `tools/map_preview.gd` calls it on the MapView at 13 fixture sites, so that seam cannot move. The EXTRACTION out of MapView was verified by byte-diff: an extracted run matched a pre-extraction run on all 56 `map_preview` frames — 0 differing (the harness is a strict bit-identity reference since PR #310 pinned its canvas and froze `Engine.time_scale`), plus 205 of 230 `blend_probe` frames; the 25 that moved are the `BANK_*` state, which **varies run-to-run on a clean tree too** — `blend_probe` does NOT freeze time and BANK is its only state carrying a `TIME`-scrolled navigable-river channel |
| `ui/AnnotationRenderer.gd` | Owns MapView's ANNOTATION family — the overlays that say something *about* the map rather than drawing the world: the Crisis overlay's annotations, the Terrain tab's "highlight all tiles of this type" tool, the per-faction order ROUTES, and the command-TARGETING overlay (valid-target glow / reticle / hover-ETA plate). **All four const families live here** (`TERRAIN_HIGHLIGHT_*` / `CRISIS_*` / `ROUTE_*` / `TARGETING_*`), each measured to have no executable reference outside the family; the one exception is `CRISIS_COLOR`, which MapView's `OVERLAY_COLORS` table also reads, so it is a `const CRISIS_COLOR := MapView.CRISIS_COLOR` alias — one definition, two readers. **The fifth family, `TRADE_*`, went with the trade-link overlay** — `draw_trade_overlay`, the three trade-overlay fields, the three reflective seams and the `trade_links` ingest, all removed because the sim publishes no link network (`overlay-channels.md` → "RETIRED", `docs/plan_contact_and_logistics.md`); issue #232's route-network overlay is what replaces it. It owns this family's own state (`_terrain_highlight_id`, `_crisis_annotations`, `_routes`, `_targeting` + its pulse clock); every draw command plus the shared primitives (`_hex_center` / `_hex_points` / `_hex_center_wrapped` / `_unwrapped_path_points` / `_draw_label` / `_draw_reticle` / `_hex_distance` / `_wrapped_col_delta` / `_get_adjusted_viewport_size`) and the world state it reads (`units` / `herds` / `terrain_overlay` / `tile_lookup` / `faction_color` / `active_overlay_key` / `_hovered_tile`) stay on MapView via the `_view` back-ref. **The is-this-mine test is NOT one of them and no longer lives on MapView at all**: `HudConst.is_player_unit(unit)` is a `class_name` static this file calls directly, and it fails closed — a unit dict with no `faction` key is not the player's, because faction 0 is. **THE `ROAD_*` FAMILY THAT USED TO SIT BESIDE `ROUTE_*` IS GONE FROM THIS FILE, AND THE TWO WERE NEVER THE SAME THING** — `ROUTE_*` draws ORDER PATHS, coloured by faction, which vanish with the order; the ROADS IN THE GROUND (arc #532) are world objects coloured by rung, and they are now painted by `terrain_blend.gdshader`'s road pass off a per-hex `road_map` splatmap, because a road is infrastructure IN the ground rather than ink laid over it. `draw_road_network` / `_draw_road` / `_road_rung_opacity` / `_road_rung_width` / `_road_tile_known` and the `ROAD_OPACITY_*` / `ROAD_WIDTH_*` / `ROAD_AT_RISK_*` / `ROAD_TILE_RADIUS_FACTOR` consts went with it. **Do not re-add a road draw here, and do not rename `ROUTE_*` into road vocabulary**; the whole arc is `.claude/rules/client/roads.md`'s. **TWO PUBLIC SEAMS KEEP SAME-NAMED PASS-THROUGHS ON MAPVIEW** because both are reached REFLECTIVELY — a rename would not error, it would silently do nothing: `set_targeting` (Main.gd connects the HUD signal by name via `has_method`/`Callable`) and `set_terrain_highlight` (TerrainPanel.gd via `has_method`/`call`). The pass-throughs store here and MapView owns the `queue_redraw` (the `set_labor_pending` idiom); `set_terrain_highlight` returns a bool so its CONDITIONAL redraw survives the move. `_targeting_time` is advanced from MapView's `_process` via `advance_targeting_time`, still gated on `is_targeting_active()`. Pixel-identical to the old inlined code, **verified by byte-diff**: an extracted run matched a pre-extraction run on **all 60 `map_preview` frames and all 230 `blend_probe` frames — 0 differing** — and, crucially, four of those `map_preview` frames were **added first** (`map_trade_overlay` / `map_crisis_annotations` / `map_terrain_highlight` / `map_routes`), because before them this family had NO pixel coverage at all and the byte-diff would have proved nothing about it. `map_trade_overlay` has since gone with the overlay it covered; the other three stand |
| `ui/BandSourceList.gd` | **The selected band's SOURCES, as a list docked beside its token** (issue #650) — a `Control` on its own `CanvasLayer` (`LAYER_INDEX` 100: above the map's `Node2D` at 0, below every docked HUD surface, `Main.HUD_LAYER` 101 … `OverlayPicker.POPOVER_CANVAS_LAYER` 105), created and owned by `MapView` in `_ready` on `MinimapPanel.setup(parent, layer_index)`'s shape. **It derives nothing**: every figure is `BandOverlayRenderer.compute_source_rows`' and a row's icon is the marker's own face through `SecondaryMarkerRenderer.face_for_*`, because a row and its hex are joined by a literal leader line and two producers of one number would be visible in a glance. `icon · ⚒N · rate · build-or-attention`, ten rows a page, a `‹ ›` pager (**never a scroll** — the wheel is already zoom), and a TWO-LINE footer: the pager, the `<page>/<pages>` readout, the whole source count and the `Work tab ▸` link on one strip, and the band's whole income on a full-width strip of its own under it, stated as ACCOUNTS (`SourceForecast.yield_components`, never one summed number). **THE TOTAL EARNS ITS OWN LINE BY ARITHMETIC** — it names every account the band earns, so it is structurally the widest string the panel draws and it grows with the BAND rather than with the page; sharing the controls strip it rendered as `+2.65 /turn · +0.40 fodder · +…`, eliding the third account away, and the shipped `Work tab ▸` link takes another 74 px off that same strip. A total the player cannot read is not the total whose INVARIANCE ACROSS PAGES is the whole justification for putting one on a paged list, and an 18 px line is cheaper than the ~80 px of width that would have fixed it sideways. `total_text()` is public so a probe compares the line itself rather than parsing it back out of a joined footer — which is how the paging guard came to read the wrong label. **The rate cell, the detail cell and both footer lines ELIDE with the whole run on a tooltip** — the retired pill sized itself to its measured run and could not overflow, and a row's cell is a fixed column, so a two-material rate (`+0.22 hide · +0.10 fibre`, ~130 px against `RATE_WIDTH` 96) drew straight through its neighbour; the string is never SHORTENED, since stating fewer materials than the source pays is the one thing this cell may not do. **A page step re-renders the FOOTER too and emits `page_changed`**, which `MapView` redraws on: the footer carries the `<page>/<pages>` readout and the pager's disabled states, and the leader lines belong to the map, so a page turned without both leaves `1/2` showing on page 2 with every line still pointing at the page that left. Row click PANS (`MapView.focus_on_tile`) and deliberately **not** `focus_and_select_tile`, which would clear `selected_unit_id` and close the list. **Its geometry is arithmetic, in `place()`, never read back off Control layout** — layout is deferred a frame and `row_anchor()` must answer in the SAME frame the leader lines are drawn in, so it is not an `AutoSizingPanel` (that helper fits a card to measured CONTENT; this height is a pure function of the page's row count). Placement is Ray's quadrant rule off `MapView.unreserved_screen_rect()` with a `DEAD_ZONE_FRACTION` band around the centre, the sides held in members so the list cannot flip while the player pans a few pixels, then clamped into the room. **`place()` takes the band's whole inked FOOTPRINT, not its centre** (`MapView._selected_band_avoid_rect` — the token's box unioned with `BandMarkerRenderer.name_pill_offset`), because a gap measured from the centre put the panel on the band's nameplate; the quadrant test still uses that rect's centre, so the rule itself is unchanged. `MapView.local_to_canvas` / `canvas_to_local` / `local_rect_to_canvas` are the ONE conversion pair between the map's draw space and the panel's |
## A marker's NAME comes from the sim, and `_rebuild_unit_markers` counts nothing

`marker["id"]` — what the Occupants drawer titles itself with and what the selection card's roster row
reads — is `HudFormat.band_name(entry)`, the client's one band-naming rule. It used to be a counter
this loop kept over the RAW `populations` array (parties included), plus a dedup loop that bumped the
counter on a collision, while `HudFormat` numbered the expedition-FILTERED roster; one live party was
enough to make the map and the Assign Hunters picker call one band two different things. Names are
unique within a faction by construction and a band's own party is told apart by the `(Scout)` suffix
`band_name` appends, so there is nothing left to count and no collision to dedup.

The `entry.get("label", "")` this replaced read a key the wire has never sent, so it always missed;
`nearest_unit_label` reads the stamped `id` for the same reason. The full rationale is in
`hud-modules.md` → "One band, one name".
