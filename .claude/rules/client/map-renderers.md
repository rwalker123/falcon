...
## Select-then-cycle — one hex, one stack of OCCUPANTS

Re-clicking the already-selected hex advances the selection through everything standing on it.
`_occupants_on_tile` **is** that stack: every `_units_on_tile` entry, then every `_herds_on_tile`
entry, each tagged `{kind, data}` (`OCCUPANT_KEY_*` / `OCCUPANT_KIND_*`). Bands-then-herds is the
click ORDER contract — a band still wins the first click on a shared hex — and composing the stack
out of those two existing helpers rather than re-matching coordinates is what makes **both fog
gates** (a foreign band under fog via `_unit_hidden_by_fog`, a herd on an unseen hex via
`_is_tile_visible`) hold here by construction.

The stack is the cycle because a band-only cycle made herds unreachable. `_handle_entity_selection`
...
at any number of clicks (issue #429). **The LAND is deliberately not in the cycle** — an empty hex
still clears to the land card, and the land is reached from the panel's land row.

**The advance is derived from the SELECTED OCCUPANT'S IDENTITY, not from the stored index.**
`_selected_occupant_index` finds `selected_unit_id` / `selected_herd_id` in the stack and the click
takes the next entry, falling back to `cycle_index` only when nothing on the hex is selected. That
keeps the map click coherent with a panel roster-row click (pick Wildlife row 3 in the panel,
re-click the hex, get row 4) and survives the occupant array reordering between snapshots.
