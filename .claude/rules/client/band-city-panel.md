...
**The fix is a shared bound, NOT a `ScrollContainer`** — and that still holds for the BAND zone. The
no-scroll rule has since been **narrowed rather than deleted**: the parties LIST scrolls (see "The
strip's height is 360 at ONE band column and 335 at two"), and `_assert_scroll_only_in_the_parties_list`
keeps the rule everywhere else. A scrolling band zone would hide vitals behind a gesture on the one
surface a player reads every turn; a parties list that can genuinely run to any length is the opposite
case, which is why one earned the exception and the other did not.
