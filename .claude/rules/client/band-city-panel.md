...
- **Header chrome.** Settlement **stage glyph + name + stage label + the band's hex coordinates**
  ...
  **THE SUBJECT ROW CARRIES NO ACTIONS — they live on the ACTION BAR one row below** (see "The action
  bar is a REGISTRY"). What stays here is the subject, the cycler over it, and the two WINDOW
  controls, which act on the panel rather than on the band.
  **The `⚒` Materials & Crafting launcher CARRIES NO SUBJECT, and that is why it is panel chrome at
  all.** The bar is subject-independent, so ONE button serves a band page and the faction page and the
...
## The action bar is a REGISTRY, not a layout

The card's content column is **title → actions → tabs → content**. Row one is the subject (stage glyph
+ name + stage label + coordinates), the `◀ n/N ▶` cycler and the two WINDOW controls (dock chooser,
collapse caret). Row two is the **ACTION BAR**, and every verb the panel offers lives on it.

**A ROW-ONE ACTION MAKES THE PANEL'S WIDTH A FUNCTION OF ITS CHROME.** That row's minimum width is
`subject + every control on it`, and the card's minimum is that plus `PANEL_CHROME_H` — so while the
`⚒` sat there the card's floor was **364px against the 380px a vertical dock reserves**, with a
one-button margin left. A second action would have taken it to **402** and forced the card wider than
its own reservation, and each one after that would have widened the dock again. With the actions moved
off, the row measures **302** and the card **326**, and — measured through the real
`register_action` — a second action moves neither by a pixel (`band_panel_preview`'s
`_assert_action_bar_registry`). **The width is now bounded by the SUBJECT, which is content.**

**The seam is a registration, so the next action is one line and nobody has to think about the panel's
width again** — the reserved-edge registry's shape, and its reason:
...
- **`enabled`** is a zero-argument `Callable` answering `bool`; an EMPTY one means always enabled. It
  ...
  keeps the bar's geometry out of reach of band state.
...
permanent tax on a panel with no actions.

### The bar is charged to the STRIP, never to the zone's budget

One row of `_make_icon_button` glyphs measures **44px** — `ACTION_BAR_MARGIN_V` (4, half the body
gutter) either side of the ghost button's own 36. Against the band zone's ~300px box that is 15%, so
where it is charged is the whole question, and `_horizontal_panel_height()` answers it the way
`_shell_chrome_height()` already did: **a third chrome term added to the strip.** `PANEL_HEIGHT_WIDE`
is the budget the zones' content is tuned against — the SHORT tier reads 299 of 300 — so a bar spent
out of a fixed strip height would have clipped the flank on its first frame.

Measured on a horizontal dock: the strip goes **360 → 404** at one band column and **335 → 379** at
two, the band zone's box is **unchanged**, and the growth is still purely geometric (the bar's height
is a function of what is REGISTERED, never of what a band holds — the flicker invariant holds).
Registration republishes the reservation, the `set_rail_width` contract, because it is a DECLARED
input made at wiring time rather than per snapshot.

**A VERTICAL dock has no strip to grow**, its height being the window's, so there the bar is paid by
the zone. That is free at any real window height and is only visible to a probe pinned to a short
canvas: `band_panel_preview`'s `COMPACT_TIER_PROBE_HEIGHT` went **480 → 524** with the bar, and it has
to move again for any row added to or removed from the card's content column, or the probe silently
slides into the tier below and asserts against the wrong one.
