...
### "START A LIFE HERE" IS THE THIRD ARRIVAL ACTION (issue #510, `docs/plan_band_fission.md` §Q2)

A party in `AwaitingOrders` can stop being an expedition and become a resident band where it stands.
It sits beside *send onward* and *recall*, and it is offered on **every surface recall is** — the
parties zone's row, that zone's inspector strip, and the Occupants drawer's action row — off the ONE
predicate `BandPanelController.party_may_settle`, exactly as the three recall surfaces read
...
with a sentence's room takes `Start a life here`, the phrase the design doc names the action by. Both
live in `HudComposeVocab`, so no surface can invent a third.

- **The row control sits BEFORE the ✕**, which keeps the removal the row's LAST child (what
  `band_panel_preview._assert_recall_press` identifies it by) and reads constructive-then-destructive.
  It is **HEALTHY green** against the ✕'s DANGER: this press MAKES a band where that one dissolves a
  party.
- **IT ALWAYS CONFIRMS.** A recall's cancel branch acts straight off the press because a party still
  in camp has lost nothing and can be re-launched; a founding is the first act in the band economy
...
- **The command carries the `BandId` and nothing else.** `settle_expedition <faction>
  ...
  server would resolve to nothing at all, silently. `cargo xtask command-guard` drives the row's own
  control and parses the emitted line with the real server parser, which is the only thing that can
  assert either half.

**The frames are a PAIR and a one-sided frame proves nothing** — a control that rendered
unconditionally passes the positive half. `band_panel_settle_offered` / `band_panel_settle_withheld`
render ONE roster (an arrived scout and a party still hunting) in the tall LEFT dock and differ only
