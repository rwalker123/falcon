...
- **Header chrome.** Settlement **stage glyph + name + stage label + the band's hex coordinates**
  ...
  **WHERE THE VERBS RENDER DEPENDS ON THE DOCK'S ORIENTATION** — on this row between the cycler and
  ...
  action registry is ONE list with TWO mount points"). What is on this row in BOTH orientations, in
...
## The action registry is ONE list with TWO mount points, chosen by the dock's orientation
...
**The actions RE-HOME when the dock edge changes at runtime.** `set_dock` calls
`_refresh_action_mount()` before laying out, and the rebuild is wholesale — both rows are cleared and
the live one filled — so nothing can be left behind on the mount the panel just moved off. It is a
no-op when the orientation is unchanged, which is what keeps an ordinary layout pass from rebuilding
(and so re-`disabled`ing) every button.
...
nothing in — the bar would take a slice of the strip on every horizontal dock, and the header row a
slice of the very width the seam protects.
...
- **No orientation argument, ever.** A caller must not know or care which mount is live; adding one
  would make every call site restate a layout rule the panel already holds in `_action_mount_for_dock`.
