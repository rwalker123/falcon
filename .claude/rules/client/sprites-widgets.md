...
| `ui/HudSprites.gd` | Bundled PNG art for **HUD MARKS** — the **seventh** art family behind `IconSprites`, and the first whose subjects are not on the MAP or in a tile-card row at all: these are drawn on the HUD's own chrome, over `HudStyle.PANEL_SOLID` at 13-24px. `for_mark(mark_id) -> Texture2D` is a direct table hit like `StageSprites`' — the key is a client-side CONSTANT (`HudKnowledgeVocab.LAUNCH_MARK`), not a server field and not a resolved free-text label, because the mark belongs to a piece of chrome rather than to a thing in the world. **It carries `cairn` ONLY, and that is COMPLETE coverage**: `assets/icons/hud/` also holds `forage.png`/`hunt.png`/`scout.png`, which are shipped art awaiting issue #249's emoji→sprite migration and are deliberately absent from the table — a path with no call site is dead data, and listing one would make this family's `warn: true` a warning about art nobody loads. Takes the DEFAULT `warn: true` (unlike `FloraSprites`) for exactly that reason: every key here has a committed, imported PNG behind it, so a failed load is a defect. **Two host widgets, two mechanisms, ONE key** — a `Button.icon` on the knowledge launcher (`BandCityPanel._make_icon_button`) and a `TextureRect` swapped in for a `Label` on the turn orb's `knowledge_learned` row; the choice is made by what the host IS, never by preference, and both fall back to `HudKnowledgeVocab.LAUNCH_GLYPH` (`▲`) when the load returns `null`. Verify on `ui_preview`'s `knowledge_launcher_mark{,_rail,_bar}.png` (the three action mounts) and `turn_orb_knowledge.png` (the orb rows) |
