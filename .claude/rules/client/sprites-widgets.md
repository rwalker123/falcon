...
paths:
  ...
  - "clients/godot_thin_client/src/scripts/ui/{CropRoleSprites,FloraSprites,HudSprites}.gd"
...
| `ui/MinimapPanel.gd` | Minimap component for the 2D map view (click-to-pan, aspect ratio sizing), plus the map-overlay picker docked on its top border — see `map-renderers.md` → "Component (`ui/MinimapPanel.gd`)" |
...
zone-head totals, the map's per-source rates — so a rate could never be mistaken for food. **ONE glyph for the
