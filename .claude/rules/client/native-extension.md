...
| `PopulationCohortState.loadoutWindow:BandLoadoutWindowState` | `loadout_window` on the band dict — **one band's outfitting window**, inserted only when the table rode the frame (absent and shut both mean *nothing to outfit*, so no third reading exists for a missing key to be confused with). `parent_band_id == 0` is a GRANT capped by the two budgets; anything else is a TAKE on that band capped by `parent_item_supply` / `parent_material_supply`, each already `holdings + this take's standing units` and listing only items some kit carries (a bench tool stays with the workshop that built it, so it is never published as claimable). Its `kits` / `materials` are the ACCEPTED allocation and are non-empty on a fresh splinter — the split's kit-denominated default take, which is what a card opens on. Its `kits` / `materials` reuse `dict/campaign.rs`'s pre-fill row decoders — one shape, one decoder | `dict/population.rs` → `loadout_window_to_dict` |
