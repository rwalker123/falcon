...
- **The Item cell's second line is a join of published fields, never an authored table**: a TOOL
  ...
  characteristic its input is judged on (`inputs[].reads_axis`), and a KIT row names the craft that
  makes it, at the sim's own `display_name`. A join that finds nothing renders no second line.
- **THE STOCK LINE NAMES NO MATERIAL, AND LEADS WITH THE CRAFT LIKE EVERY OTHER ROW** —
  `Tanning · quality from suppleness`. It read `Reads hide suppleness` until it was reported as
  ...
  `4 wood · 2 hide`, and the sim's own refusal under the button says `Hide, no tanning frame → poor`.
  What is genuinely new on the line is the AXIS, which is the whole of *there is no best hide*: the
  same pelt makes a good sled (reads toughness) and poor hurdles (reads suppleness). Leading with the
  craft is also what keeps line two a CATEGORY on every row — the stock row was the only one whose
  line two was a sentence, which is part of why it read as an error.
