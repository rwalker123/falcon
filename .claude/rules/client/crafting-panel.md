...
neighbour"). **The bar moves under an open card** — it toggles on `R`, flips edge and changes row
count — so `CraftingPanelController.refit_room()` re-fits on every publication; a room that only
bit at opening time leaves the reported frame exactly as broken.
