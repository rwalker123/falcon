...
- **A STOCK ROW REPORTS THE BAND'S OWN PILE OF THE MATERIAL IT MAKES, batch by batch, in the RAIL'S
  own row** — the amount, then the ratings that batch is held at, one line per batch. This cell stated
  the recipe's YIELD (`→ 1 hurdles`) on the premise that *a stock recipe owns nothing*, and the
  premise was simply false: a crafted material is banked in `material_batches` like a gathered one,
  and the same panel was drawing the band's 5.0 hurdles in the rail 250px to the left while this
  cell — under a column head reading **Owned** — reported a number that was not an amount. It was
  reported from play as *"the count isn't working"*, which is the reading the column head invites.
  `_build_batch_row` is REUSED rather than restated: the rail and this cell answer one question about
  one pile, and two builders for it are two renderings free to disagree.
- **A MATERIAL IS MEASURED, NOT COUNTED — no `×` on a stock row.** Equipment is `×26` sleds; a pile is
  `22.8` hide and `6.5` cordage, at the rail's own one decimal. A cell reaching for the equipment
  column's `×n` renders `×7` for 6.5, which is the shape the harness fixture's un-whole amounts exist
  to catch. A material the band banks NONE of says `None on hand`, the pile's version of the
  `Bare hands` / `Not made` sentence beside it.
- **THE YIELD RIDES WITH THE COST — `4 wood · 2 hide → 1 hurdles`.** `inputs → output` is what an
  arrow means; alone in the Owned cell it named a relation with nothing on its left, which is why it
  read as a badge nobody could parse. Rendered on a MATERIAL output and on no other — a kit recipe's
  output is the row's own name, and an arrow back at the title says nothing.
