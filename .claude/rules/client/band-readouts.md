...
| `ui/BandFoodStatus.gd` | Single source of truth for band food-supply thresholds (`band_status_config.json`) + the days→green/amber/red color / BBCode-hex mapping (plus the parallel morale and output warn/critical thresholds, each with the `color_for_*`/`hex_for_*` pair a `Label` host and a BBCode host respectively need — `color_for_output` is the WORK zone head's), shared by MapView's band dot and Hud's food/morale lines + alerts |
...
- **Civilization Wellbeing — productivity, itemized morale, recovery** (see
  ...
  - **Productivity is NOT a detail row — it reads on the Band panel's WORK zone head** as
    ...
    **`BandFoodStatus.color_for_output`**, the `Color` twin of `hex_for_output` (the
    `color_for_morale`/`hex_for_morale` pairing); the two must stay a pair so the panel's two
    productivity surfaces cannot grade one multiplier differently. `DetailFormat.Context.output` and
    the renderer's `"Output"` tint branch went with the row — no emitter survived — so nothing in the
    detail path carries the scalar any more. The head item's placement, gate and vocabulary are
