...
| `ui/MenuShell.gd` | The Options pane's "Interface scale" row — the first row in the pane, since it governs the legibility of every row under it. Writes `ClientSettings` **and stops**. Its arrival is why `_make_speed_slider_row` takes a `step` parameter: the builder is the pane's general slider row now, and a type-size increment is not a speed granularity. **The row reads in PERCENT** (`75%` … `150%`), not the speed rows' `%.2f×` — a UI scale is a percentage everywhere else a user has met one. That is a per-row `format_readout` `Callable`, carried on the slider's meta beside the label, because the "Restore defaults" path reads the label back off meta rather than through the builder: a formatter passed only to the builder renders `135%` on drag and `1.00×` after a reset |
