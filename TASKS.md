...
- [ ] Band/City panel — multi-column content fill when docked wide (top/bottom) (Owner: TBD, Estimate: 1d; Deps: `docs/plan_band_city_dock.md`, `BandCityPanel.gd`). Description: On an ultrawide top/bottom dock the bounded two-column layout leaves large empty space on the right and reads poorly. Make the band content (summary + the allocation's discrete sections — Current actions / Band roles / Orders / Send expedition / Active expeditions) flow into multiple columns to fill the strip. Needs restructuring `Hud._build_allocation_panel`'s flat output into discrete section blocks (its stepper/band-picker/pending wiring is delicate — preserve it) so the panel can lay them out as flowable fixed-width columns. Deferred from the initial Band/City dock PR (which shipped bounded-columns-packed-left as a stopgap).
