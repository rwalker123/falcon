...
| **Draw down to survive** | the ledger's projection of the plan in force troughs below zero before the upgrade pays | lower the harvest floor on a worked patch below Best, as far as the profile's `survival_floor` (the forager's is `0` — survival outranks the peak), to the highest floor whose projection stays above zero; the reason names the floor and the turn the drawn-down patch is expected to be spent. The goal floor stays Best; this is the dial a starving band turns, and it turns back once the projection clears |
