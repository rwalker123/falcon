...
- **Next Steps (UI plumbing)**:
  ...
  - Inspector overlays now ship normalized, raw, and contrast rasters plus live min/avg/max readouts in the legend so designers can validate colour ramps against ledger telemetry without the retired CLI inspector (see `shadow_scale_strategy_game_concept_technical_plan_v_0.md` “Map-Centric Evaluation Plan”).
