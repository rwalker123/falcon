...
- Relief markers: the 3D `UnitOverlay3D` consumes `heightfield_config.json -> markers` for runtime tuning. Designers can toggle marker visibility, adjust scale and `y_offset`, and flip `shaded` to switch between unlit icons (legend style) and lit meshes (reacting to scene lights) while keeping palette parity with the 2D MapView legends.
...
- Founding: `Command::FoundSettlement { q, r }` is implemented. It requires a `Founders` starting unit on the faction, consumes provisions from `FactionInventory`, despawns the founders, and spawns a `Settlement` + `TownCenter` (construction/logistics radii) at the target tile. The command records a `found_settlement` event and re-centers fog/survey radius around the founded hub. For the nomadic path it should only be exposed once sedentarization ≥ threshold and a camp is upgraded in place.
