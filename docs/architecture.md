...
- **Implementation Status (v0.1)**:
  ...
  - Command surface now exposes `queue_espionage_mission`, wiring directly into `EspionageMissionState::queue_mission` so remote clients can schedule missions without waiting for scripted turn logic.
  ...
  - Follow-up work: surface mission queue commands via the Godot client, extend outcomes (partial successes, misinformation hooks), and wire counter-intel missions to actively clear infiltration records.
