...
- You begin as the lead of a Late Forager Tribe: a **single small band** (~30 people) that is your entire **labor pool**. Rather than commanding separate Scout/Hunter/Guardian *bands*, you partition the band's working-age population across equipment-gated **roles** — Foraging, Hunting, Scouting, Warrior (Crafting arrives with the tool-making economy). The band splits into more bands as it grows — and it must, because a band alone cannot grow past what its founding families can sustain: growth means meeting other bands, and the game's first walk is for people, not food. The world is shrouded by Fog of Discovery: tiles are unknown until observed; resources and chemistry hints appear only after local surveys and experiments. See `docs/plan_early_game_labor.md` for the labor-pool / equipment model that drives the opening.
- Initial kit: food rations plus a starter **Table of Equipment** for each role (baskets, spears/traps, wayfinding gear, weapons), portable camp gear (tents, drying racks), and a knowledge kit enabling basic assays (kiln/burn/solvent tests). Equipment is **consumable** and, until crafting comes online, cannot be replaced — a role whose kit runs dry falls back to a weaker bare-hands tier. No permanent buildings exist yet.
...
- Wildlife & Hunting (shipped — see `docs/plan_wildlife_hunting_overlay.md`): wild game is an overlay on the map rather than a tile flag, modeled as short-range **herds** (the same fauna system as migratory herds, just with a smaller roaming range — big game ~2–3 tiles, small game ~1). A tile can therefore offer **both** gathering *and* hunting (a forest has nuts *and* deer). Two source-centric assignments: **Forage** (the tile's gather module) and **Hunt** (a herd), each carrying a **policy**. **Your workers go where the food is** (designed — see `docs/plan_civilization_steps.md`): assigning people to Hunt or Forage is the only command. A hunting party follows its herd on its own, because that is where the meat is; a foraging party stands on its patch, because the patch does not move. Near the band, what they take pools into the band's larder by itself; far away, it has to be **hauled home**, and the hauling wears a trail into the ground that becomes a route. Unassign them and they walk home. One hunt, one forage, and no expedition to outfit — the same people, just somewhere else. For a herd the four harvest policies are **how hard you push it** — each takes more per turn than the last, and the harder you push the faster the herd falls: *Sustain* takes only what the herd renews, so it **holds steady and feeds you forever** (a single herd sustainably feeds only a handful, so a real population draws from many at once); *Surplus* takes **half again as much**, so the herd **slowly declines** — reversible if you ease back to Sustain; *Market* takes **more than double**, at a boosted trade rate, and drives the herd **to extinction**; *Eradicate* wipes it out now. **You kill whole animals**, and the land sets how fast they come: your hunters bank the herd's yield until it's worth a whole beast, so **small fast game comes many-a-turn while a mammoth is a windfall you wait weeks for** — and a harder policy just fills that bank faster. So the choice is honest: a herd that lasts, or more meat now at the cost of the herd. *(See the Intensification Ladder — the land, tamed or wild, sets the renewal rate the policies multiply.)* Plus the Ladder's investment rungs above (*Cultivate*/*Sow*, *Tame*/*Corral*), which pay less now to pay more later. Together with whole-animal harvest and land-driven regrowth, this is the substrate for the long-term overhunting/depletion arc — abundant early, thinning as population pressure and commercial (*Market*) hunting grow, unless domestication takes hold. This supersedes the current single-action Follow Herd below and retires the static `game_trail` food-site.
- Predators & the Danger of the Hunt (designed — see `docs/plan_predators.md`): not every animal is meat you simply take. A predator is an animal like any other — it just **eats other animals** instead of grass, so it goes where the game is, **follows** the herds, competes with you for the very deer you were counting on, and **fades when the game is gone** (no one has to cull the wolves — hunger does; their numbers rise with the herds and crash with them). And the hunt itself can **cost you people**. Every beast carries a **defense** — a deer only runs, but a mammoth gores and an aurochs charges, so the biggest game is a **windfall you can be killed collecting** — and a predator carries **teeth** on top of it: it will raid your unguarded foragers unprovoked. So *which* beast a hunter can take is honest — a lone forager cannot bring down a mammoth any more than a wolf can. You can turn on the predators and **wipe them out** (the *Eradicate* you already know), but that is a fight, and a fight is **a fight like any other in this world** — resolved by who you send and what they carry. The hunt's danger is the **hunters' own** to answer: send them after a mammoth bare-handed and you will bury some of them; put **spears** in their hands and the same hunt comes home whole — the danger falls with the tools you make, not with a guard you send along. **Warriors** are a different thing — a role that produces no food and stands watch over the **camp**, for the day a predator comes hunting *you*: they guard the band against the raid, not the hunter against the beast. *(The wolf that shadows your hunts may, in far time, come to heel and hunt at your side — but that is a later and stranger story.)*
...
### The Intensification Ladder (Wild → Tended → Rooted)
Every food source — plant or animal — climbs the **same ladder**, driven by the same grammar. This is the bridge from "we eat what we find" to "we make our food," and it is where most of your early labor decisions live. See `docs/plan_intensification_ladder.md` for the model.

- **Two paths, one grammar.** *Plants:* a wild forage patch → **Cultivate** it into a **Tended Patch** → **Sow** a **Field** on ground that will already take seed → (later) **work the land** so ground that *won't* take seed finally does. *Animals:* a wild herd → **Tame** it into a **Pastoral** herd → **Corral** it into a **Pen** → (later) **breed** it into better stock. Same shape both sides: a verb you choose deliberately, not a thing that happens to you.
- **You farm where the land is willing, before you make it willing.** You have learned to take seed from a plant and put it somewhere else — but not yet to feed or water the ground, so the ground must do that itself: sowing works only on land that is *already rich and already watered* — the floodplain, the river delta, the alluvial valley. Thin ground and dry ground stay shut until you learn to *work* land (the plow, the ditch), which is a later and much larger step. Such country is **rare** — a thin green thread along the rivers — so your first fields are a real choice about *where*, and a people may have to **move** to farm at all. That pull toward the good country is how you stop wandering.
- **Every rung is an investment that dips before it pays.** Choosing the verb *lowers* your yield while you work — you're handling, preparing, fencing, not harvesting. A build meter climbs; **stop and it decays** (a patch goes feral, a half-tamed herd drifts back to wild, an untended pen breaks down). When it completes, the source pays more.
- **Intensifying buys density; tools buy freedom.** A Field is not a gift — it is a job. Climbing the ladder does not mean fewer hands per meal; it means **far more food from one patch of ground**, which is why a band stops walking and settles around it. What actually frees a person from food work is **technology** — a better basket, a granary, a plow — because a worker can only carry so much home, however rich the land. (This is what really happened: early farming was *harder* work per calorie than foraging. It won because it fed far more people from one valley.)
- **Production and collection are two different numbers.** The ground offers X; your workers can carry Y. You take the smaller one, and the rest **rots in the field**. So a rich Field wants more hands than a thin one, and an understaffed harvest visibly wastes what it cannot carry — until better tools raise what each pair of hands can bring home.
- **Climbing raises four things:** how much the **land** yields, how fast the source **regrows**, how much it can **hold**, and — for animals — how **close** it stays. A wild herd roams far and you chase it; a tamed herd drifts along with your band; a penned herd stands at your fence. Far → near → fixed.
- **Knowledge comes from practice, never from a menu.** Working a rung teaches you the next one. Sustainably hunting wild game teaches **Herding**; taming herds teaches **Penning**. Sustainably foraging teaches **Cultivation**; tending patches teaches **Seed Selection**. Nothing is granted at start — you learn a thing by doing the thing below it, so the ladder is paced by your actual play.
- **Not every animal climbs.** Some are hunt-only however long you work them (deer, mammoth — you eat them, you do not herd them). Some you can herd but never fence (the migratory grazers — you follow them, as herders always have). Only some go all the way to a pen (boar, aurochs, goats). Each species declares how far it can go.
- **Rung 2 is the fork that decides whether you settle.** A pastoral herd *travels with you* — nomadism keeps working. A tended patch *is a place* — it roots you. Rung 3 pins **both** sides: a Field and a Pen are things you cannot carry. That is the moment the settlement on-ramps below stop being a prompt and start being a fact.

### What Grows There — Named Plants (Staples, Fodder, Cash)
The land does not offer "food." It offers *hazel and acorn under the canopy*, *reeds and tubers in the delta*, *seed heads on the steppe*. Every place has a **basket** — the particular plants that grow there — and the basket, not a number, is what you learn to read. See `docs/plan_flora_roster.md` for the model.

- **Foraging takes the whole basket; farming takes one thing.** A wild patch feeds you everything on it, and choosing a place is choosing a basket. But when you **Cultivate** or **Sow**, you commit that ground to *one plant* — and the rest of what grew there gives way. That is the real cost of a field, and it is why tending rich mixed country is sometimes worse than leaving it wild.
- **A crop is worth the land, or it is not.** Each plant thrives in particular country and is thin or absent elsewhere. There is no plant that is right everywhere, and on poor ground there is nothing worth the labor at all. What to sow is a question the *place* answers, not your tech.
- **Not every plant climbs.** Some are a wild harvest forever — you gather the oak's mast, you do not sow an oak. Others tend, and only some take seed and become a field. Each plant declares how far it can go, exactly as each animal does.
- **Fodder is how your fields come to feed your herds.** A pen eats the grass inside its own fence, so its size is bound to its pasture. Grow a **feed crop** — hay — and you can *store* it: carry a herd through the lean season, or keep more animals than the fence's grass alone would hold. That is mixed farming, and it is the step that let livestock leave the pasture. It costs you a field to do it; hauling your own bread to the animals remains as poor a bargain as it has always been.
- **Some plants are not food at all.** Tobacco, cotton, sugar, tea: they pay in trade, not calories, and they want the same rare rich, watered ground your grain does. Every acre of it is a plain choice — *eat, or sell*.

### Before the Village — People, Belief, and the Dead
Long before anyone farms, the game asks the question the whole early loop turns on: *why would a people stop walking?* Food is the reason you **can** stay. These are the reasons you **want** to — and the one reason you can't. (designed — see `docs/plan_civilization_steps.md` for the model)

- **A band alone cannot grow.** Every band descends from a handful of **founding lines**, and a people cannot grow past what those lines can sustain. A splinter that walks off and stays away caps low and stays small. What lifts the ceiling is **contact** — meeting other bands, your own far bands or strangers — so keep a split within reach, or on a trade route, and both halves keep growing. And when a band is crowded or cut off, the people who leave it **walk the routes** to a connected band. Nobody vanishes into nowhere; they go where your trails go.
- **The gathering.** When bands meet, the world notices. It is told in The Telling and announced on the turn orb the way a discovery is — there is no command to give, you simply brought your people together. Where bands *keep* meeting, something accrues on that ground.
- **Belief is a place.** A tile gains belief from the dead you bury there, from the gatherings held there, and later from a **monument** raised on it — the first thing you will ever build that feeds no one. It never fades; an abandoned place keeps its dead. Standing near your belief lifts morale; walking away from your ancestors drags it, and the morale breakdown says so in plain words. Belief moves your people toward the Devout and the Traditionalist, and it feeds the tether that draws you to settle. Religion stays a matter of culture, not a separate system.
- **Cohesion.** A band, or a cluster of bands on one site, above roughly a hundred and fifty people starts to **leak**: people drift to another band along the routes, because no one can know everyone anymore. Belief on the ground you stand on raises that ceiling — a sacred place is what lets one place hold more than a band. Writing, much later, lifts it again.
- **The push.** Standing still eats the patch down — the overdraw the source rows already show. So the decision the early game is about is never abstract: the ancestors and the partners say *stay*, the thinning patch says *go*.

### The Granary — Food That Rots, and a Place That Keeps It
The first thing a surplus teaches you is that you cannot keep it. (designed — see `docs/plan_civilization_steps.md` for the model)

- **Food rots.** Everything you hold loses a little each turn, and different foods keep differently — meat and fish go fast unless cured; grain, seeds and nuts keep for a season if they stay dry. There is no limit on what a band can carry; the rot is the limit, so a wandering band's larder settles where what spoils equals what comes in. The Food line shows the rot as its own term, so you always know where the food went.
- **Watching food rot teaches you to keep it.** A band that never has more than it eats never learns storage. A band with a surplus watches some of it spoil, and that is the lesson — storage is learned by practice, like every rung of the ladder. Two first answers: a **drying rack**, which travels with you and cures meat and fish; and a **pit or granary**, which does not travel, holds far more, and keeps only dry goods. Sealed pottery comes later, from craft.
- **A granary is a place.** What you put in a pit is on the ground, not on your backs. A band within reach of its store draws on it; walk beyond reach and it is behind you, spoiling slowly, waiting. That is the first stored surplus that is *somewhere* rather than *with someone*, and it pulls at you the way the ancestors do.
- **A fed band accepts direction.** Work that feeds no one — a monument, later a wall — is possible only while the granary can feed the people doing it, and it stops when the granary empties. How long a band will put up with being pointed at such work is a matter of its culture: an egalitarian people mock a would-be chief long before a hierarchical one would. Every such work finished while the people stayed fed makes the next one easier to ask for; that is how a chief becomes a line of chiefs, and The Telling will say so. Nobody is ever handed a crown.
...
- Sedentarization Score (0–100): emergent pressure to root in place. Inputs: local resource density (now including the live herd-density overlay), food stability, storage/spoilage reductions, domestication progress, trade hub potential, travel fatigue, security, **belief on the ground** (the dead, gatherings, a monument) and the **presence of partner bands** within reach.
...
- Multiple settlement on-ramps:
  ...
  - Cultural/religious path: **sacred ground is a real input, not flavor** — the dead you buried, the gatherings held there and a monument raised on it lift morale for those who stay and drag it for those who leave; population lingers and builders stay.
...
- Capability flags: Great Discoveries now flip simulation capability bits (power, espionage tier 2, megaprojects) to unlock the corresponding schedules; these bits stream in snapshots so clients can gate UI/actions.
...
- Map presets: scenario-selectable map presets (e.g., Earthlike) define macro shape and target proportions of tag categories (Water, Fertile, Wetland, Hazardous, etc.). Presets can also nominate a short list of `locked_terrain_tags`; the solver focuses on keeping those families within tolerance (great for promising farmland or wetlands) while letting everything else drift naturally. The builder still nudges marginal tiles with adjacency rules, but only applies adjustments to locked tags so we avoid a tug-of-war across every category. Presets can pin their own world seed; use `seed_policy = preset_fixed` with a `map_seed` to lock in a signature arrangement. The `Polar Microplate Contrast` preset does this and leans on a higher microplate density/uplift pair so the caps pop with alternating ridges and flats.
  - `terrain_classifier.polar_latitude_cutoff` sets where the climate band flips from temperate to polar. Lowering it brings tundra tags closer to the equator; raising it reserves polar tags for the extreme caps. This feeds the same threshold used by `bias_terrain_for_preset` and the climate solver, so the preset’s value keeps biome swaps and player-facing climate bands in sync. See `docs/architecture.md` §“Terrain Classifier Knobs” for implementation notes.
...
- Moisture modeling now flows from coast to interior with wind-aware rain shadows: coastal tiles get bonus rainfall that decays inland, latitude bands set prevailing winds with jitter, windward slopes gain extra lift, and leeward corridors dry out over several tiles. A range's shadow begins at its **crest**, so the windward flank and the summit stay wet and the dry corridor only starts on the descent. Scenario presets expose these knobs (`prevailing_wind_flip_chance`, `windward_moisture_bonus`, `rain_shadow_strength`, `rain_shadow_decay`) so designers can push for monsoon tropics or stark rain shadows; see the engineering notes in `docs/architecture.md`.
...
Great Discovery definitions now flow from the simulation server to the inspector with each snapshot, keeping player-facing metadata in lockstep with the authoritative catalog. Designers can mark numeric gates as fixed values or `{min,max}` bands; the server samples once per load using deterministic seeds (see `docs/architecture.md` §Great Discovery Catalog for loader specifics and `seed_offset` usage) so campaign replays stay reproducible while still allowing gentle variability.
...
- Simulation and world defaults (grid size, temperature curves, power tuning, corruption multipliers, and TCP bind ports) now live in `core_sim/src/data/simulation_config.json`. Editing the JSON reshapes the baseline map, environmental ticks, and server endpoints before the prototype boots.
  - `grid_size`, `population_cluster_stride`, `population_cap`: map extent and cohort spacing.
  - `ambient_temperature`, `temperature_lerp`, `power_adjust_rate`: thermal relaxation and power-temperature coupling.
  - `migration_fragment_scaling`, `migration_fidelity_floor`: the knowledge-diffusion curve for **migration**, the one inter-faction diffusion path that was ever live. The trade/logistics tuning that sat beside these (`logistics_flow_gain`, `base_link_capacity`, `mass_bounds`, `mass_flux_epsilon`, `base_trade_tariff`, `trade_leak_*`) is gone with the dead trade slice — see `docs/plan_contact_and_logistics.md` §As-built, and §Q4 for the connection-and-riders model that replaces it.
...
### Counter-Intel Budgets & Policy Hooks
- **Defensive Reserve**: Each faction earmarks a `Counter-Intel Budget` that fuels rapid-response sweeps and network audits. When the ledger flags high-risk discoveries (≥70% leak progress or active infiltrations), the simulation automatically spends from this reserve to queue defensive missions if funds remain. Budget shortfalls delay or cancel the sweep, surfacing a warning in the Knowledge Ledger panel.
...
- **UI Hooks**: The Knowledge tab includes inline controls for selecting faction doctrine (Lenient/Standard/Hardened/Crisis) and adjusting the counter-intel reserve (set or delta). A mission queue form sits above the event log so operators can trial sweeps or probes with auto-agent selection, target tier overrides, and custom schedule offsets.
...
Cross-System Integration: Culture traits feed the Sentiment Sphere, Logistics (through compliance with quotas), Diplomacy (via trust/openness), and Military doctrine (training morale). Future updates should ensure tasks mirror these hooks in the architecture doc and in the backlog issues.

### Designer Tuning & Telemetry
- Culture tension fallout is now data-driven via `core_sim/src/data/culture_corruption_config.json`. The `culture.trust_axis` field declares which sentiment axis absorbs drift and assimilation adjustments, while each block (`drift_warning`, `assimilation_push`, `schism_risk`) specifies the severity clamps and delta curves (scale/min/max) applied before we write incident deltas into `SentimentAxisBias` and archive the same event in `DiplomacyLeverage.culture_signals`. Designers can dial these scalars to make early warnings gentler or schism risks harsher, and inspector telemetry will always reflect the post-clamp delta.
- The same file’s `culture.propagation` block controls how quickly layers converge and how long divergence must persist before the simulation raises alarms. `elasticity` tunes the lerp per scope (global/regional/local), the `soft_threshold`/`hard_threshold` pairs define drift vs. schism magnitudes, and `soft_trigger_ticks` / `hard_trigger_ticks` act as grace periods so designers can stretch assimilation windows or delay schism escalations to match narrative pacing.
- The same config’s `corruption` section governs scandal fallout: `sentiment_delta_min/max` bounds the trust hit once an incident matures, `max_penalty_ratio` caps how much corruption can choke logistics/trade/power throughput, and `min_output_multiplier` guarantees infrastructure keeps a survivable trickle. Adjustments here keep diplomacy sandboxes consistent with the implementation described in `docs/architecture.md` §"Corruption Simulation Backbone".
- `core_sim/src/data/turn_pipeline_config.json` captures hot knobs for each turn stage designers keep tinkering with: population pace modifiers (terrain penalty scales, morale thresholds, growth clamp, migration ETA), and power smoothing (efficiency floor/adjust rate, influence demand reduction, storage efficiency/bleed ceilings). Use the CLI command `reload_config turn [path]` (or edit the watched file) to hot-load new values while reviewing telemetry.
- `core_sim/src/data/snapshot_overlays_config.json` now drives the inspector overlays: tweak corruption channel weights/spike multipliers, culture divergence boosts, military presence/support blending, and fog-of-war averaging without touching Rust. Reload via `reload_config overlay [path]` to iterate on color ramps and legends live.
