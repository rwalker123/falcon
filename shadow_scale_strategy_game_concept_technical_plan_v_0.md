...
- Wildlife & Hunting (planned redesign — see `docs/plan_wildlife_hunting_overlay.md`): wild game becomes an overlay on the map rather than a tile flag, modeled as short-range **herds** (the same fauna system as migratory herds, just with a smaller roaming range — big game ~2–3 tiles, small game ~1). A tile can therefore offer **both** gathering *and* hunting (a forest has nuts *and* deer). Three decoupled verbs: **Harvest** (the tile's gather module), **Follow** (shadow a group and stay within ~1 tile, pursuing it as it moves), and **Hunt** (take a portion of the group's biomass). Follow carries an auto-hunt **policy** that sustains the **resource**, not the band — *Sustain* takes only the herd's per-turn regrowth (you eat the interest, the herd stays stable — so a single herd sustainably feeds only a handful of people, and a real population must draw from many sources at once), *Surplus* (take extra to sell, herd slowly declines), *Eradicate* (drive to local extinction) — which, together with per-group biomass draw-down and regrowth, is the substrate for the long-term overhunting/depletion arc (abundant early, thinning as population pressure and market hunting grow unless domestication/industrialized hunting take hold). This supersedes the current single-action Follow Herd below and retires the static `game_trail` food-site.
