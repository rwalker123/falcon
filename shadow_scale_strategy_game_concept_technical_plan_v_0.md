...
- Wildlife & Hunting (shipped — see `docs/plan_wildlife_hunting_overlay.md`): wild game is an overlay on the map rather than a tile flag, modeled as short-range **herds** (the same fauna system as migratory herds, just with a smaller roaming range — big game ~2–3 tiles, small game ~1). A tile can therefore offer **both** gathering *and* hunting (a forest has nuts *and* deer). Two source-centric assignments: **Forage** (the tile's gather module) and **Hunt** (a herd), each carrying a **policy** that sustains the **resource**, not the band — *Sustain* takes only the source's per-turn regrowth (you eat the interest, it stays stable — so a single source sustainably feeds only a handful of people, and a real population must draw from many at once), *Surplus* (take extra to sell, it slowly declines), *Market* (commercial offtake at a boosted trade rate — it crashes slow breeders fastest), *Eradicate* (drive to local extinction) — plus the Intensification Ladder's investment rungs above (*Cultivate*/*Sow*, *Tame*/*Corral*), which pay less now to pay more later — which, together with per-group biomass draw-down and regrowth, is the substrate for the long-term overhunting/depletion arc (abundant early, thinning as population pressure and market hunting grow unless domestication/industrialized hunting take hold). This supersedes the current single-action Follow Herd below and retires the static `game_trail` food-site.
