...
- Wildlife & Hunting (shipped — see `docs/plan_wildlife_hunting_overlay.md`): wild game is an overlay on the map rather than a tile flag, modeled as short-range **herds** (the same fauna system as migratory herds, just with a smaller roaming range — big game ~2–3 tiles, small game ~1). A tile can therefore offer **both** gathering *and* hunting (a forest has nuts *and* deer). Two source-centric assignments: **Forage** (the tile's gather module) and **Hunt** (a herd), each carrying a **policy**. For a herd the four harvest policies are **the size you manage the herd down to** — you take whole animals until it reaches that size, then only what regrows above it: *Sustain* holds the herd at its **most productive size** (you crop the renewal and it stays there forever — a single herd sustainably feeds only a handful, so a real population must draw from many at once); *Surplus* keeps a **smaller herd** and pockets the difference (a big haul while it draws down, a thinner yield after); *Market* strips it to a **collapsed remnant** for a bigger haul still, at a boosted trade rate — a herd too small to feed a people; *Eradicate* takes everything. Each option takes **more than the one before it** — because you can't kill a fraction of an animal, this is whole beasts down to a target, not a percentage skim. *(You harvest a herd; you don't slice it. The land still sets how fast it refills — see the Intensification Ladder — so a fat herd hands over a burst and a lean one makes you wait.)* Plus the Ladder's investment rungs above (*Cultivate*/*Sow*, *Tame*/*Corral*), which pay less now to pay more later. Together with whole-animal draw-down and land-driven regrowth, this is the substrate for the long-term overhunting/depletion arc (abundant early, thinning as population pressure grows — where *Market* driving slow breeders to extinction while fast ones endure will live). This supersedes the current single-action Follow Herd below and retires the static `game_trail` food-site.
