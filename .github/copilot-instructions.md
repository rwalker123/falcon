...
- **Respect capability gating.** Systems run under `run_if(capability_enabled(...))`, which is an `intersects` test — *any* listed bit passes, not all. Most registrations OR in `ALWAYS_ON`, and `CapabilityFlags::default()` is `ALWAYS_ON`, so a `FLAG | ALWAYS_ON` gate runs by default; only a system gated on its own flag alone is genuinely inert until that flag is set. New subsystems should gate behind the appropriate flag rather than always running — and a `| ALWAYS_ON` on a subsystem that is meant to be optional is worth flagging, since it makes it unconditional.
