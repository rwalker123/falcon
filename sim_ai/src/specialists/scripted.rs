//! **`Scripted`** — the test fixture as a specialist (`docs/plan_ai_driver.md` §1, §4): replays a
//! fixed command list, one proposal per line that fires, with intent `script` and score
//! [`SCRIPT_SCORE`], through a pass-through arbiter. That keeps the smoke test, the scenario test
//! and the real opponent on one code path, and keeps the decision records exactly as slice 3
//! wrote them (`specialist: "scripted"`, `intent: "script"`, both scores 1.0, accepted).
//!
//! ## The script format
//!
...
/// The one intent a script line carries — the bare word, no class, exactly as slice 3 wrote it.
pub const SCRIPT_INTENT: &str = "script";
/// The score a script line is logged at, raw and final alike: nothing outscores a script, and
/// the pass-through arbiter never weighs it.
...
mod tests {
    use super::*;
    use sim_runtime::{CommandPayload, PopulationCohortState, WorldSnapshot};
    ...
    fn propose(specialist: &mut Scripted, tick: u64) -> Vec<Proposal> {
        specialist
            .propose(
                &a_view_at(tick),
    ...
    #[test]
    fn a_line_fires_on_its_tick_as_one_proposal_with_its_placeholders_resolved_and_only_once() {
        let mut specialist = Scripted::from_script(
            &format!("{A_TICK}: split_band {{faction}} {{own_band:1}} 4"),
            OUR_FACTION,
        )
        .expect("parses");
        assert!(propose(&mut specialist, A_TICK - 1).is_empty());
        let proposals = propose(&mut specialist, A_TICK);
        assert_eq!(proposals.len(), 1);
        assert_eq!(
            proposals[0].commands,
        ...
        assert_eq!(proposals[0].intent, SCRIPT_INTENT);
        assert_eq!(proposals[0].score, SCRIPT_SCORE);
        assert_eq!(
            proposals[0].reason,
            format!("split_band {OUR_FACTION} {OUR_SECOND_BAND} 4")
        );
        assert!(propose(&mut specialist, A_TICK + 1).is_empty());
    }

    #[test]
    fn a_relative_tick_counts_from_the_first_tick_the_specialist_saw() {
        let mut specialist =
            Scripted::from_script("+1: split_band {faction} {own_band:0} 4", OUR_FACTION)
                .expect("parses");
        assert!(propose(&mut specialist, A_TICK).is_empty());
        assert_eq!(propose(&mut specialist, A_TICK + 1).len(), 1);
    }
